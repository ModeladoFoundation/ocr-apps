...
BINARY_BASENAME=${BINARY_BASENAME-"z_ptest_reductionEvent"}
...
function build {
    ...
    for config in `echo ${TUPLES_CONFIGURATIONS}`; do
        ...
        echo "vars=${LIST_TUPLE_VARIABLES} entries=${entries}"
        ...
        eval "CFLAGS=\"-DCOUNT_FOR_FORKJOIN=${R_VAL} -DARITY=${A_VAL} -DREDUC_OPERATION_TYPE=${T_VAL} -DRECURB_ITER_COUNT=${RECURB_ITER_COUNT}\" ${ADD_MAKEFLAGS} RUN_MODE=buildApp TARGET=${TARGET_NAME} V=1 make install"
    done
}
