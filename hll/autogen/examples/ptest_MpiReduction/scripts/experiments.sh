...
function fatTwoLevelConfigUnderSubscribe {
    ...
    TUPLES_CONFIGURATIONS=${tuples}
    export BINARY_BASENAME="z_ptest_reductionEventUnderSubscribed"
}
...
function kAryTreeConfig {
    ...
    TUPLES_CONFIGURATIONS=${tuples}
    export BINARY_BASENAME="z_ptest_reductionEventBinary"
}
...
#TODO:
#- build OCR
#- double check each build has different binary names
#- how does fatTwoLevelConfigFixedArity differentiate from fatTwoLevelConfig ?
#- compare 2016 and 2017 mpi
#- compare srun with and without affinities
function nerscRun {
    export TPLARG_QUEUE=regular
    export BATCH_TPL=reduction-nersc
    export TPLARG_MIN=05
    export CORE_SCALING_SAVE="${CORE_SCALING}"
    export OCR_INSTALL=${OCR_INSTALL-${SCRATCH}/ocr_install_mpiicc-072717-redevt-nodebug}
    export REDUC_OPERATION_TYPE=${REDUC_OPERATION_TYPE-REDOP_ALLREDUCE}
    fatTwoLevelConfigFixedArity
    PRFIX="${SCRATCH}/ocr_install_mpich-"
    SIMPLE_OCR_NAME=`echo $OCR_INSTALL | sed -e "s|${PRFIX}||g"`
    CPGN=binary-tree-${SIMPLE_OCR_NAME}-${REDUC_OPERATION_TYPE}-redevt-edison-fatTwoLevelConfigFixedArity
    for CMD in `echo ${CMDS}`; do
        doExperiment
    done
    if [[ -n "${LOG_CPGN}" ]]; then
        echo "$PWD/$CPGN" >> ${LOG_CPGN}
    fi
    ...
    for mc in `echo ${CORE_SCALING}`; do
        ...
        PRFIX="${SCRATCH}/ocr_install_mpich-"
        SIMPLE_OCR_NAME=`echo $OCR_INSTALL | sed -e "s|${PRFIX}||g"`
        CPGN=cpgn-${SIMPLE_OCR_NAME}-${REDUC_OPERATION_TYPE}-redevt-edison-binaryTreeConfigCoreScaling-${mc}c
        for CMD in `echo ${CMDS}`; do
            doExperiment
        done
        if [[ -n "${LOG_CPGN}" ]]; then
    ...
    PRFIX="${SCRATCH}/ocr_install_mpich-"
    SIMPLE_OCR_NAME=`echo $OCR_INSTALL | sed -e "s|${PRFIX}||g"`
    CPGN=binary-tree-${SIMPLE_OCR_NAME}-${REDUC_OPERATION_TYPE}-redevt-edison-binaryTreeConfigSingleCore
    export c=${CORE_SCALING}
    for CMD in `echo ${CMDS}`; do
        doExperiment
    done
...
function thorRun {
    export OCR_INSTALL=${OCR_INSTALL-$HOME/ocr_install_mpiicc_redevt-nodebug}
    ...
    SIMPLE_OCR_NAME=`echo $OCR_INSTALL | sed -e "s|${PRFIX}||g"`
    CPGN=${CPGN-binary-tree-${SIMPLE_OCR_NAME}-${REDUC_OPERATION_TYPE}-redevt-fatTwoLevelConfigFixedArity}
    for CMD in `echo ${CMDS}`; do
        doExperiment
    done
    ...
    for mc in `echo ${CORE_SCALING}`; do
        ...
        PRFIX="$HOME/ocr_install_mpiicc_"
        SIMPLE_OCR_NAME=`echo $OCR_INSTALL | sed -e "s|${PRFIX}||g"`
        CPGN=cpgn-${SIMPLE_OCR_NAME}-${REDUC_OPERATION_TYPE}-redevt-binaryTreeConfigCoreScaling2-${mc}c
        for CMD in `echo ${CMDS}`; do
            doExperiment
        done
    done
    ...
    export c=${CORE_SCALING}
    export OCR_INSTALL=${HOME}/ocr_install_mpiicc_redevt-nodebug
    binaryTreeConfigSingleCore
    PRFIX="$HOME/ocr_install_mpiicc_"
    SIMPLE_OCR_NAME=`echo $OCR_INSTALL | sed -e "s|${PRFIX}||g"`
    CPGN=binary-tree-${SIMPLE_OCR_NAME}-${REDUC_OPERATION_TYPE}-redevt-binaryTreeConfigSingleCore2
    export c=${CORE_SCALING}
    for CMD in `echo ${CMDS}`; do
        doExperiment
    done
