...
static void setup_HTA(int nprocs, int pid, HTA** u_HTA, HTA** v_HTA, HTA** r_HTA);
static void mg3P(HTA** u_HTA, HTA* v_HTA, HTA** r_HTA);
...
static void zran3(void *oz, int nx1, int ny1, int nz1, HTA* v_HTA);
...
int hta_main(int argc, char *argv[], int pid)
{
  ...
  setup_HTA(PROC, pid, u_HTA, &v_HTA, r_HTA);
  zran3(v_array, nx[lt], ny[lt], nz[lt], v_HTA);
  comm3_HTA(v_HTA); // barrier before pulling in data from other threads
  norm2u3_HTA(v_HTA, &rnm2, &rnmu, nx[lt], ny[lt], nz[lt], lt);
  //if (pid == 0) printf(" L2 Norm is %20.13E\n", rnm2);
  ...
  if (Class != 'U') {
    ...
    if (err <= epsilon) {
      ...
      MASTER_PRINTF(" L2 Norm is %20.13E\n", rnm2);
      MASTER_PRINTF(" Error is   %20.13E\n", err);
    } else {
      verified = false;
      MASTER_PRINTF(" VERIFICATION FAILED\n");
      MASTER_PRINTF(" L2 Norm is             %20.13E\n", rnm2);
      MASTER_PRINTF(" The correct L2 Norm is %20.13E\n", verify_value);
    }
  } else {
    verified = false;
  ...
  if (timeron) {
    ...
    char rec_name[256];
    sprintf(rec_name, "rec_spmd/mg.%c.%d.rec", Class, PROC);
...
void setup_HTA (int nprocs, int pid, HTA** u_HTA, HTA** v_HTA, HTA** r_HTA)
{
  int di[3];
  int ov[3];
  int ng[MAXLEVEL+1][3];
  ...
  for (int k = lt; k >=1; k--)
  {
    ...
    r_HTA[k] = HTA_create_with_pid(pid, 3, 2, &fs, 0, &dist0, HTA_SCALAR_TYPE_DOUBLE, 1, t0);
    u_HTA[k] = HTA_create_with_pid(pid, 3, 2, &fs, 0, &dist0, HTA_SCALAR_TYPE_DOUBLE, 1, t0);

    // there is only one (the finest) grid for v_HTA
    if(k == lt) {
        *v_HTA = HTA_create_with_pid(pid, 3, 2, &fs, 0, &dist0, HTA_SCALAR_TYPE_DOUBLE, 1, t0);
...
static void rprj3_serial(HTA* s_tile, double* fine_grid, int fine_grid_di[3]) // s is the coarser one
{
    int j3, j2, j1, i3, i2, i1, d1, d2, d3;
    double x1[M], y1[M], x2, y2;

    int m3k = fine_grid_di[0];
    int m2k = fine_grid_di[1];
    int m1k = fine_grid_di[2];
    int m3j = s_tile->flat_size.values[0];
    int m2j = s_tile->flat_size.values[1];
    int m1j = s_tile->flat_size.values[2];

    double (*r)[m2k][m1k] = (double (*)[m2k][m1k])fine_grid;
    double (*s)[m2j][m1j] = (double (*)[m2j][m1j])HTA_get_ptr_raw_data(s_tile);
    if (m1k == 3) {
      d1 = 2;
    } else {
...
// Reading element value from possibly remote HTA
...
void _adjust(double *tmp_a, int di[3]) {
    int x, y, z;
    int di_r[3]; // real data size
    for (int i = 0; i < 3; i++) {
        di_r[i] = di[i] - 2;
        //printf("di[%d] = %d, di_r[%d] = %d\n", i, di[i], i, di_r[i]);
    }
    double (*a)[di[1]][di[2]] = (double (*)[di[1]][di[2]])tmp_a; // reinterpret the pointer so it's easier to index
    double (*b)[di_r[1]][di_r[2]] = (double (*)[di_r[1]][di_r[2]])tmp_a; // reinterpret the pointer so it's easier to index
    // shift data location first
    for(z = di_r[0] - 1; z >= 0; z--)
        for(y = di_r[1] - 1; y >= 0; y--)
            for(x = di_r[2] - 1; x >= 0; x--) {
                a[z+1][y+1][x+1] = b[z][y][x];
                //printf("read (%d, %d, %d) and write to (%d, %d, %d)\n", z, y, x, z+1, y+1, x+1);
            }
   // read boundary
   for(y = 1; y < di[1] - 1; y++) {
       for(x = 1; x < di[2] - 1; x++) {
           a[0][y][x] = a[di[0] - 2][y][x];
           a[di[0]-1][y][x] = a[1][y][x];
	   //printf("boundary xy surface (%d, %d, %d) and write to (%d, %d, %d)\n", di[0]-2, y, x, 0, y, x);
	   //printf("boundary xy surface (%d, %d, %d) and write to (%d, %d, %d)\n", 1, y, x, di[0]-1, y, x);
       }
   }
   for(z = 0; z < di[0]; z++) {
       for(x = 1; x < di[2] - 1; x++) {
           a[z][0][x] = a[z][di[1] - 2][x];
           a[z][di[1] - 1][x] = a[z][1][x];
	   //printf("boundary zx surface (%d, %d, %d) and write to (%d, %d, %d)\n", z, di[1]-2, x, z, 0, x);
	   //printf("boundary zx surface (%d, %d, %d) and write to (%d, %d, %d)\n", z, 1, x, z, di[1]-1, x);
       }
   }
   for(z = 0; z < di[0]; z++) {
       for(y = 0; y < di[1]; y++) {
           a[z][y][0] = a[z][y][di[2] - 2];
           a[z][y][di[2] - 1] = a[z][y][1];
	   //printf("boundary zx surface (%d, %d, %d) and write to (%d, %d, %d)\n", z, y, di[2]-2, z, y, 0);
	   //printf("boundary zx surface (%d, %d, %d) and write to (%d, %d, %d)\n", z, y, 1, z, y, di[2]-1);
       }
   }
...
static void rprj3_HTA(HTA* r_HTA, HTA* s_HTA, int k)
{
  ...
  else { // process s_HTA serially
      int di[3];
      int num_elems = 1;
      for(int i = 0; i < 3; i ++) {
          di[i] = r_HTA->flat_size.values[i] - r_HTA->tiling.values[i] * 2 + 2;
          num_elems *= di[i];
      }
      Region rhs_elem_sel;
      Range rhs_elem_range_x, rhs_elem_range_y, rhs_elem_range_z;
      CREATE_ELEM_SELECTION(rhs, 1, -2, 1, -2, 1, -2);
      double* tmp_a = malloc(num_elems * HTA_get_scalar_size(r_HTA));
      HTA_flatten(tmp_a, NULL, &rhs_elem_sel, r_HTA); // collaboratively fill the array with overlap region removed
      // sequentially process this projection
      MASTER_EXEC_START
          //printf("serial rprj %d -> %d\n", k, k-1);
          //_adjust(tmp_a, tmp_b, di);
          _adjust(tmp_a, di);
          rprj3_serial(s_HTA->tiles[0], tmp_a, di); // FIXME: this can cause deadlock if not all processes execute
      MASTER_EXEC_END
      free(tmp_a);
  }
  MASTER_EXEC_START
...
static void interp_serial(HTA* z_tile, double* u_tile, int di[3])
{
  int mm3 = z_tile->flat_size.values[0];
  int mm2 = z_tile->flat_size.values[1];
  int mm1 = z_tile->flat_size.values[2];
  ...
  int n1 = di[2];
  double (*z)[mm2][mm1] = (double (*)[mm2][mm1])HTA_get_ptr_raw_data(z_tile);
  double (*u)[n2][n1] = (double (*)[n2][n1])u_tile;
...
static int x_di[3]; // using an array that's only updated by thread 0 to pass information to a serial function
static void distribute_interp_serial_results(HTA* u_tile, void* x_tile)
{
    // iterate through u_tile elements and add appropriate offset values
    // to get the interpolation results
    ...
    int m2 = x_di[1];
    int m1 = x_di[2];
    //printf("distribute interp (%d, %d, %d) array to (%d, %d, %d) tiles\n",
    //           m3, m2, m1, n3, n2, n1);
    double (*zu)[n2][n1] = (double (*)[n2][n1])HTA_get_ptr_raw_data(u_tile);
    double (*zx)[m2][m1] = (double (*)[m2][m1])(*(double**)x_tile);
...
static void interp_HTA(HTA* z_HTA, HTA* u_HTA, int k)
{
  ...
  else
  {
    int x, y, z;
    int di[3];
    x_di[0] = di[0] = z_HTA->flat_size.values[0] * 2 - 2;
    x_di[1] = di[1] = z_HTA->flat_size.values[1] * 2 - 2;
    x_di[2] = di[2] = z_HTA->flat_size.values[2] * 2 - 2;
    SET_SPMD_PID(z_HTA->pid);
    MASTER_EXEC_START
        if(z_tmp == NULL)
            z_tmp = malloc(di[0]*di[1]*di[2] * HTA_get_scalar_size(z_HTA));
        double (*zz)[di[1]][di[2]] = (double (*)[di[1]][di[2]])z_tmp;
    	for(z = 0; z < di[0]; z++)
            for(y = 0; y < di[1]; y++)
                for(x = 0; x < di[2]; x++)
                    zz[z][y][x] = 0.0f;
        interp_serial(z_HTA->tiles[0], z_tmp, di);
    MASTER_EXEC_SYNC_END
    //HTA_tile_to_hta(HTA_LEAF_LEVEL(u_HTA), distribute_interp_results, u_HTA, u_HTA, x_HTA);
    HTA_map_h1s1(HTA_LEAF_LEVEL(u_HTA), distribute_interp_serial_results, u_HTA, &z_tmp);
  }
...
static void rep_nrm(HTA* u_HTA, char *title, int kk)
{
  ...
  MASTER_PRINTF(" Level%2d in %8s: norms =%21.14E%21.14E\n", kk, title, rnm2, rnmu);
}
