...
void pintgr_HTA(int pid, int pi[3], double *frc, HTA *u_HTA)
{
  //---------------------------------------------------------------------
  // local variables
  //---------------------------------------------------------------------
  ...
  Dist dist0;
  Tuple mesh;
  Tuple_init(&mesh, 4, pi[2], pi[1]*pi[0], 1, 1);
  //Tuple_init(&mesh, 4, PROC_Z, PROC_Y, PROC_X, 1);
  Dist_init(&dist0, DIST_CYCLIC, &mesh);
  phi1_HTA = HTA_create_with_pid(pid, 4, 2, &fs0, 0, &dist0, HTA_SCALAR_TYPE_DOUBLE, 1, t0);
  phi2_HTA = HTA_create_with_pid(pid, 4, 2, &fs0, 0, &dist0, HTA_SCALAR_TYPE_DOUBLE, 1, t0);

  HTA_map_h1s1(HTA_LEAF_LEVEL(phi1_HTA), H1S1_INIT, phi1_HTA, &zero);
  HTA_map_h1s1(HTA_LEAF_LEVEL(phi2_HTA), H1S1_INIT, phi2_HTA, &zero);

  check_bound = -2;
  //sync_boundary(pid, u_HTA);
  ...
  *frc = 0.25 * ( frc1 + frc2 + frc3 );
  ...
  HTA_destroy(phi1_HTA);
  HTA_destroy(phi2_HTA);
  HTA_destroy(r1);
  HTA_destroy(r2);
  HTA_destroy(r3);
  HTA_destroy(r4);
