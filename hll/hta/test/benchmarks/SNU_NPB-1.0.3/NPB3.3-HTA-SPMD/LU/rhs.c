...
//---------------------------------------------------------------------
void rhs_HTA(int pid, HTA *u_HTA, HTA *rsd_HTA, HTA *frct_HTA, HTA *qs_HTA, HTA *rho_i_HTA)
{
    SET_SPMD_PID(pid)
    ...
    MASTER_EXEC_END
    HTA_map_h5(HTA_LEAF_LEVEL(rsd_HTA), rhs_compute_HTA, frct_HTA, rsd_HTA, u_HTA, rho_i_HTA, qs_HTA);

#ifdef SPMD_PIL
    //MASTER_EXEC_START
    //if (timeron) timer_start(t_sync_rhs);
    //MASTER_EXEC_END
    //if(CFG_get(CFG_ALLOW_SHARED_DATA)) {
	//HTA_barrier(pid);
    //}
    //else {
    //  sync_boundary_assign(rsd_HTA);
    //}
    //MASTER_EXEC_START
    //if (timeron) timer_stop(t_sync_rhs);
    //MASTER_EXEC_END
#endif
    MASTER_EXEC_START
...
void rhs_compute_HTA(HTA* s1_tile, HTA* s2_tile, HTA* s3_tile, HTA* s4_tile, HTA* s5_tile)
{
...
#ifdef SPMD_PIL
//     if(CFG_get(CFG_ALLOW_SHARED_DATA)) {
//       sync_boundary_HTA(s2_tile, ptr_rsd_HTA[s2_tile->pid]);
//     }
#else
    sync_boundary_HTA(s2_tile, ptr_rsd_HTA[0]);
#endif
}
