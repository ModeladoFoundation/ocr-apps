...
//
//---------------------------------------------------------------------
void buts_HTA(int iter, int pid, HTA *u_HTA, HTA *rsd_HTA, HTA *qs_HTA, HTA *rho_i_HTA)
{
  int w;
  Tuple t;
  Tuple_init_zero(&t, 1);
  ...
  for(w = iter; w >= 0; w--) { // Wavefront propagation to calculate rsd_HTA
	...
	//HTA_barrier(pid);
      //}
      //else {
	sync_boundary_assign_sel(rsd_HTA, iter, w, -1);
      //}
#endif
      MASTER_EXEC_START
...
void jacu_buts_compute2_HTA(HTA* s1_tile, HTA* s2_tile, HTA* s3_tile, HTA* s4_tile)
{
  ...
  for(k = k_last; k >= k_first; k--) {
	...
	for (i = i_first; i < i_last; i++) {
	  //for (m = 0; m < 5; m++) {
	    u_tile[k][j][i][0] = u_tile[k][j][i][0] + (tmp * rsd_tile[k][j][i][0]);
	    u_tile[k][j][i][1] = u_tile[k][j][i][1] + (tmp * rsd_tile[k][j][i][1]);
	    u_tile[k][j][i][2] = u_tile[k][j][i][2] + (tmp * rsd_tile[k][j][i][2]);
	    u_tile[k][j][i][3] = u_tile[k][j][i][3] + (tmp * rsd_tile[k][j][i][3]);
	    u_tile[k][j][i][4] = u_tile[k][j][i][4] + (tmp * rsd_tile[k][j][i][4]);
	  //}
	}
      }
    }
...
#ifdef SPMD_PIL
//     if(CFG_get(CFG_ALLOW_SHARED_DATA)) {
//       sync_boundary_HTA(s1_tile, ptr_rsd_HTA[s1_tile->pid]);
//     }
#else
    sync_boundary_HTA(s1_tile, ptr_rsd_HTA[0]);
#endif
  //} // end if
