...
void init_hta(int pid, HTA ** ptwiddle_HTA, HTA** pu0_HTA, HTA** pu1_HTA, HTA** pu2_HTA, HTA** pchecksum_HTA)
{
  ...
  HTA *u1_HTA = HTA_create_with_pid(pid, 3, 3, &fs0, 0, &dist1, HTA_SCALAR_TYPE_DCOMPLEX, 2, t0, t1);
  HTA_map_h1s1(HTA_LEAF_LEVEL(u1_HTA), H1S1_INIT, u1_HTA, &dcomplex_zero);

  Tuple fs1 = Tuple_create(3, NP, NP, 1);
  HTA *checksum_HTA = HTA_create_with_pid(pid, 3, 3, &fs1, 0, &dist1, HTA_SCALAR_TYPE_DCOMPLEX, 2, t0, t1);
...
static void print_timers()
{
  ...
  char rec_name[256];
  sprintf(rec_name, "rec_spmd/ft.%c.%d.rec", CLASS, NP);
...
static void cffts1(HTA * ox_tile, void* dir)
{
  ...
  dcomplex (*ty1)[FFTBLOCKPAD_DEFAULT] = (dcomplex (*)[FFTBLOCKPAD_DEFAULT])ty1a[ox_tile->pid];
  dcomplex (*ty2)[FFTBLOCKPAD_DEFAULT] = (dcomplex (*)[FFTBLOCKPAD_DEFAULT])ty2a[ox_tile->pid];
