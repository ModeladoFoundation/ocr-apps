...
void full_verify(HTA* key_buff1_HTA, HTA* last_rank_HTA );
...
void local_count_keys(HTA * key_buff1_leaf, HTA* key_buff2_leaf, HTA* bucket_size_leaf, HTA* bucket_ptrs_leaf, void* s1)
{
    int j, b, k;
    int id = *(int*)s1;
    INT_TYPE* key_counters = HTA_get_ptr_raw_data(key_buff1_leaf);
    int key_offset = id * (MAX_KEY / PROC);
...
void final_sorting(HTA * key_buff1_leaf, HTA* last_rank_leaf)
{
    ...
    for(i = 0; i < num_keys; i++)
    {
        ...
        {
            key_array[j] = i + key_offset; // FIXME: key_array should be HTA? here it's a globally accessable array
        }
...
{
    INT_TYPE   j;
    SET_SPMD_PID(key_buff1_HTA->pid);
...
void rank( int iteration, HTA* key_array_HTA, HTA* key_buff2_HTA, HTA* bucket_size_HTA, HTA* bucket_ptrs_HTA, HTA* key_buff1_HTA, HTA* last_rank_HTA )
{
    int pid = key_array_HTA->pid;
    SET_SPMD_PID(pid);
...
/*  top of array bucket_size                                     */
    ...
    MASTER_EXEC_END
    // HTA_map_h1(HTA_LEAF_LEVEL(key_buff1_HTA), count_keys, key_buff1_HTA); // FIXME: require global access here
    // map + shift to count keys
    Tuple dir;
    Tuple_init(&dir, 1, 1);
    for(i = 0; i < PROC; i++)
    {
        //the id must be passed as an arg to the operation in order to determine the range of buckets
        int id = (pid - i + PROC) % PROC;
        HTA_map_h4s1(HTA_LEAF_LEVEL(key_buff1_HTA), local_count_keys, key_buff1_HTA, key_buff2_HTA, bucket_size_HTA, bucket_ptrs_HTA, &id);
        HTA_circshift(key_buff1_HTA, &dir);
    }
...
//    }
    ...
    int initval = 0;
    HTA_scan(key_buff1_HTA, REDUCE_SUM, &initval);
    MASTER_EXEC_START
    ...
    // FIXME: Put a barrier here before verification
    //HTA_barrier(pid);
    ...
    for( i=0; i<TEST_ARRAY_SIZE; i++ )
    {
        ...
        if( 0 < k  &&  k <= NUM_KEYS-1 )
        {
            ...
            MASTER_EXEC_START
                ...
                if( failed == 1 )
                    printf( "pid (%d) Failed partial verification: "
                            "iteration %d, test key %d, key_rank = %d\n",
                             pid, iteration, (int)i, key_rank );
            MASTER_EXEC_END
        }
    }
...
void alloc_and_init_HTAs(int pid, HTA** pkey_array_HTA, HTA** pkey_buff2_HTA, HTA** pbucket_size_HTA, HTA** pbucket_ptrs_HTA,
       HTA** pkey_buff1_HTA, HTA** plast_rank_HTA)
{
    INT_TYPE initval_0 = 0;
    /* HTA initialization */
    ...
    HTA* key_array_HTA = HTA_create_with_pid(pid, 1, 2, &fs0, 0, &dist0, SCALAR_TYPE, 1, tp0);  // tile size = TOTAL_KEYS/PROC
    HTA* key_buff2_HTA = HTA_create_with_pid(pid, 1, 2, &fs0, 0, &dist0, SCALAR_TYPE, 1, tp0);  // tile size = TOTAL_KEYS/PROC

    Tuple fs1 = Tuple_create(1, NUM_BUCKETS * PROC);
    HTA* bucket_size_HTA = HTA_create_with_pid(pid, 1, 2, &fs1, 0, &dist0, SCALAR_TYPE, 1, tp0);
    HTA* bucket_ptrs_HTA = HTA_create_with_pid(pid, 1, 2, &fs1, 0, &dist0, SCALAR_TYPE, 1, tp0);

    Tuple fs2 = Tuple_create(1, MAX_KEY);
    assert( NUM_BUCKETS % PROC == 0  && MAX_KEY % NUM_BUCKETS == 0 );
    HTA* key_buff1_HTA = HTA_create_with_pid(pid, 1, 2, &fs2, 0, &dist0, SCALAR_TYPE, 1, tp0); // tile size = MAX_KEY/PROC

    Tuple fs3 = Tuple_create(1, PROC);
    //HTA* local_sum_HTA = HTA_create_with_pid(pid, 1, 2, &fs3, 0, &dist0, SCALAR_TYPE, 1, tp0);

    HTA* last_rank_HTA = HTA_create_with_pid(pid, 1, 2, &fs3, 0, &dist0, SCALAR_TYPE, 1, tp0);
    ...
    *pkey_array_HTA = key_array_HTA;
    *pkey_buff2_HTA = key_buff2_HTA;
    *pbucket_size_HTA = bucket_size_HTA;
    *pbucket_ptrs_HTA = bucket_ptrs_HTA;
    *pkey_buff1_HTA = key_buff1_HTA;
    //*plocal_sum_HTA = local_sum_HTA;
    *plast_rank_HTA = last_rank_HTA;
}
/*****************************************************************/
/*************             M  A  I  N             ****************/
/*****************************************************************/
...
/*  Initialize timers  */
    ...
    alloc_and_init_HTAs(pid, &key_array_HTA, &key_buff2_HTA, &bucket_size_HTA, &bucket_ptrs_HTA, &key_buff1_HTA, &last_rank_HTA);
...
/*  Do one interation for free (i.e., untimed) to guarantee initialization of
    ...
    MASTER_EXEC_END
    rank( 1, key_array_HTA, key_buff2_HTA, bucket_size_HTA, bucket_ptrs_HTA, key_buff1_HTA, last_rank_HTA);
    MASTER_EXEC_START
        timer_on = timed;
...
/*  This is the main iteration */
    for( iteration=1; iteration<=MAX_ITERATIONS; iteration++ )
    {
        MASTER_EXEC_START
            if( CLASS != 'S' ) printf( "        %d\n", iteration );
        MASTER_EXEC_END
        rank( iteration, key_array_HTA, key_buff2_HTA, bucket_size_HTA, bucket_ptrs_HTA, key_buff1_HTA, last_rank_HTA );
    }
...
/*  This tests that keys are in sequence: sorting of last ranked key seq
    ...
    MASTER_EXEC_END
    full_verify(key_buff1_HTA, last_rank_HTA );
    MASTER_EXEC_START
    ...
    /*  Print additional timers  */
        if (timer_on) {
           ...
           char rec_name[256];
           sprintf(rec_name, "rec_spmd/is.%c.%d.rec", CLASS, PROC);
