...
void Grid::InitializeVerticalCoordinate(
	...
	int nComponents = GetModel().GetEquationSet().GetComponents();
...
*/
	...
	if (GetModel().GetEquationSet().GetDimensionality() == 2) {
/*
...
*/
	...
	} else if (GetModel().GetEquationSet().GetDimensionality() == 3) {
...
*/
	...
	//m_vecVarIndex.Allocate(GetModel().GetEquationSet().GetComponents());
	for (size_t c = 0; c < GetModel().GetEquationSet().GetComponents(); c++) {
		if (m_vecVarLocation[c] == DataLocation_Node) {
...
void Grid::Checksum(
	...
	if (eDataType == DataType_State) {
		dChecksumsLocal.Allocate(GetModel().GetEquationSet().GetComponents());

	} else if (eDataType == DataType_Tracers) {
		int nTracers = GetModel().GetEquationSet().GetTracers();
		if (nTracers == 0) {
			return;
		}
...
/*
	...
	if ((status.Contains(DataType_Tracers)) &&
		(GetModel().GetEquationSet().GetTracers() == 0)
	) {
...
void Grid::ReduceInterpolate(
	...
	// Check interpolation data array size
	if ((eDataType == DataType_State) &&
		(dInterpData.GetRows() != GetModel().GetEquationSet().GetComponents())
	) {
		_EXCEPTIONT("InterpData dimension mismatch (0)");
	}

	if ((eDataType == DataType_Tracers) &&
		(dInterpData.GetRows() != GetModel().GetEquationSet().GetTracers())
	) {
		_EXCEPTIONT("InterpData dimension mismatch (0)");
	}
...
void Grid::DeactivatePatch(
	...
	for (int i = 0; i < m_vecActiveGridPatchIndices.size(); i++) {
		if (m_vecActiveGridPatchIndices[i] == ixPatch) {
			TR_DELETE(m_vecActiveGridPatches[i]);
...
#if 0
                printf ("GJDEBUG: DeactivatePatch %d [%ld] \n",
				ixPatch, m_vecActiveGridPatchIndices.size());
#endif
                fflush(stdout);
	}
...
/*
	...
	const PhysicalConstants & phys = GetModel().GetPhysicalConstants();
...
/*
	...
	PhysicalConstants & phys = GetModel().GetPhysicalConstants();
