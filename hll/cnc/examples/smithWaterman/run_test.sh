...
OCR_CONFIG=${OCR_INSTALL}/config/default.cfg ./SmithWaterman.exec inputs/string{1,2}.txt 3 3 2>&1 | tee $TMPFILE
fgrep -q 'score: 10' < $TMPFILE  && echo OK || echo FAIL

echo "Testing SA.txt vs SB.txt with width=569, height=661"
OCR_CONFIG=${OCR_INSTALL}/config/default.cfg ./SmithWaterman.exec inputs/S{A,B}.txt 569 661 2>&1 | tee $TMPFILE
