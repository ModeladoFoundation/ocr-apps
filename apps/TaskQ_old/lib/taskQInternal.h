...
{
    // Jatin HACK!!!
    // Assumes 1D grid & # tasks evenly divisible by # queues
    int tasksPerQueue = max[0]/totalQueues; if ( (tasksPerQueue * totalQueues) != max[0]) tasksPerQueue++;
    int j;
    long my_min[MAX_DIMENSION], my_max[MAX_DIMENSION];

    for ( j = 0; j < totalQueues; j++)
    {
        my_min[0] = tasksPerQueue * j;
        my_max[0] = my_min[0] + tasksPerQueue;
        if (my_min[0] > max[0]) my_min[0] = max[0];
        if (my_max[0] > max[0]) my_max[0] = max[0];

        if (my_min[0] != my_max[0]) assignTasksStandardized( fn, taskFn, numDimensions, startQueue+j, my_min, my_max, step);
    }
}
}
