...
server_nodes=(217 218 219)
client_nodes=(229 230 231 232)
...
for BLOCKCOUNT in ${BLOCKCOUNTS[@]}; do
    ...
    for iter in ${iters[@]}; do
        ...
        BUILD_CMD="MACHINE_CONFIG=$PWD/${machineConfigFile} WORKLOAD_INSTALL_ROOT=./${winstall} make -f Makefile.tg install WORKLOAD_ARGS='-s ${size} -t $((8*$BLOCKCOUNT)) -l ${iter}'"
        eval $BUILD_CMD
