...
NuclideGridPoint ** gpmatrix(ocrDBK_t *DBK_nuclide_grids, ocrDBK_t *DBK_nuclide_grid_ptrs, size_t m, size_t n)
{
    ...
    ocrDbCreate( DBK_nuclide_grids, (void **) &full, m*n*sizeof(NuclideGridPoint),
                    0, NULL_HINT, NO_ALLOC );

	NuclideGridPoint ** M; // = (NuclideGridPoint **) malloc( m *
	                       //   sizeof(NuclideGridPoint *) );

    ocrDbCreate( DBK_nuclide_grid_ptrs, (void **) &M, m*sizeof(NuclideGridPoint *),
                    0, NULL_HINT, NO_ALLOC );

    assignNuclideGridPtrs( full, M, m, n);
