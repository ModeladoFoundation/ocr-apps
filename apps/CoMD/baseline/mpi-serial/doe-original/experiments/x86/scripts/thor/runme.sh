...
srchme="/home/rsalmon/dhome/fastfw/new/apps/apps/CoMD/baseline/mpi-serial/doe-original"
exehme="${srchme}/build_and_run/mip/CoMD"
...
for i in `seq $ns $ne`; do
    ...
    sbatch --nodes=${nodes} --job-name="${lname}" --output="$rhm/${lname}" --partition=XAS ${srchme}/experiments/x86/scripts/thor/simple-comd-${tn}.sh ${scale} ${rhm} ${MYITAC} ${psize} ${exehme} ${ph}
