...
static int getBoxFromCoord1(LinkCell* boxes, real_t rr[3]);
                  ...
                  const real_t px, const real_t py, const real_t pz)
{
   ...
   // Find correct box.
   int iBox = getBoxFromCoord1(boxes, xyz);
...
void updateLinkCells(LinkCell* boxes, Atoms* atoms)
{
   ...
   for (int iBox=0; iBox<boxes->nLocalBoxes; ++iBox)
   {
      ...
      {
         int jBox = getBoxFromCoord1(boxes, atoms->r[iOff+ii]);
         if (jBox != iBox)
...
int getBoxFromCoord1(LinkCell* boxes, real_t rr[3])
{
