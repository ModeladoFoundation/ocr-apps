...
void setMomentum(SimFlat_t* s, real_t temperature)
{
    ...
    for (int iOff=0, ii=0; ii<s->atoms->nAtoms; ++ii, ++iOff)
    {
       ...
       s->atoms->p[iOff][0] = mass * sigma * gasdev(&seed); //TODO
...
ocrGuid_t comdEdt(u32 paramc, u64 *paramv, u32 depc, ocrEdtDep_t depv[])
{
    ...
    real_t localVcm[4] = {0.0, 0.0, 0.0, 0.0};
...
ocrGuid_t main1Edt( u32 paramc, u64 *paramv, u32 depc, ocrEdtDep_t depv[] )
{
    ocrGuid_t cmd_g = depv[0].guid;
    ocrGuid_t sbDBK = depv[1].guid;
...
ocrGuid_t mainEdt( u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[] )
{
    ...
    ocrGuid_t main1EDT, main1TML;

    ocrEdtTemplateCreate( &main1TML, main1Edt, 0, 2 );

    ocrEdtCreate( &main1EDT, main1TML, EDT_PARAM_DEF, NULL, EDT_PARAM_DEF, NULL, EDT_PROP_NONE, NULL_HINT, NULL );

    ocrAddDependence( cmd_g, main1EDT, 0, DB_MODE_RO );
    ocrAddDependence( sharedDBK, main1EDT, 1, DB_MODE_RW );

    ocrEdtTemplateDestroy( main1TML );
