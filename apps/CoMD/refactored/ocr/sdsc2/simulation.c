...
{
  reduction_t* reduction = (reduction_t*)depv[0].ptr;
  rpf_t* rpf = (rpf_t*)depv[1].ptr;
  ocrGuid_t* schedule = (ocrGuid_t*)depv[2].ptr;

  u8 a;
  for(a=0; a<rpf->atoms; ++a) {
    rpf->p[a][0] *= reduction->value[0];
    rpf->p[a][1] *= reduction->value[0];
    rpf->p[a][2] *= reduction->value[0];
  }

  u8 n;
  for(n=0; n<26; ++n)
    ocrAddDependence(depv[1].guid, schedule[n], n+1, DB_MODE_RO);
...
ocrGuid_t vcm_edt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
  ...
  *ek_p = 0;
  ...
  for(a=0; a<rpf->atoms; ++a) {
    ...
    rpf->p[a][0] += m*reduction->value[0];
    rpf->p[a][1] += m*reduction->value[1];
    rpf->p[a][2] += m*reduction->value[2];
    *ek_p += (rpf->p[a][0]*rpf->p[a][0]+rpf->p[a][1]*rpf->p[a][1]+rpf->p[a][2]*rpf->p[a][2])*
              mass->inv_mass_2[rpf->s[a]];
  }

  ocrAddDependence(reduction->reduction, *(ocrGuid_t*)(paramv+1), 0, DB_MODE_RO);
...
//depv: sim, mass, reduction, box, rpf, sched0, .., sched26, rbp
ocrGuid_t init_edt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
  simulation_t* sim = (simulation_t*)depv[0].ptr;
  mass_t* mass = (mass_t*)depv[1].ptr;
  reduction_t* reduction = (reduction_t*)depv[2].ptr;
  box_t* box = (box_t*)depv[3].ptr;
  rpf_t* rpf = (rpf_t*)depv[4].ptr;
#ifndef TG_ARCH
  memset(box,0,sizeof(box_t)); memset(rpf,0,sizeof(rpf_t));
#else
  for(u32 m=0; m<sizeof(box_t); ++m) ((char*)box)[m]=0;
  for(u32 m=0; m<sizeof(rpf_t); ++m) ((char*)rpf)[m]=0;
#endif
  rpf->box = depv[3].guid;
  box->coordinates[0] = ((u32*)paramv)[0];
  box->coordinates[1] = ((u32*)paramv)[1];
  box->coordinates[2] = ((u32*)paramv)[2];

  box->min[0] = box->coordinates[0]*sim->boxes.box_size[0];
  box->min[1] = box->coordinates[1]*sim->boxes.box_size[1];
  box->min[2] = box->coordinates[2]*sim->boxes.box_size[2];
  box->max[0] = box->min[0]+sim->boxes.box_size[0];
  box->max[1] = box->min[1]+sim->boxes.box_size[1];
  box->max[2] = box->min[2]+sim->boxes.box_size[2];
  s32 begin[3] = {floor(box->min[0]/sim->pot.lat)-1,
                  floor(box->min[1]/sim->pot.lat)-1,
                  floor(box->min[2]/sim->pot.lat)-1};
  s32 end[3] = {ceil((box->max[0])/sim->pot.lat)+1,
                ceil((box->max[1])/sim->pot.lat)+1,
                ceil((box->max[2])/sim->pot.lat)+1};
  ...
  if(box->coordinates[0]==0) rpf->nmask|=1;
  if(box->coordinates[0]==sim->boxes.grid[0]-1) rpf->nmask|=2;
  if(box->coordinates[1]==0) rpf->nmask|=4;
  if(box->coordinates[1]==sim->boxes.grid[1]-1) rpf->nmask|=8;
  if(box->coordinates[2]==0) rpf->nmask|=16;
  if(box->coordinates[2]==sim->boxes.grid[2]-1) rpf->nmask|=32;

  ocrGuid_t* rbp = (ocrGuid_t*)depv[32].ptr;
  u8 rb;
  for(rb=0; rb < 26; ++rb) {
    box->rneighbors[rb] = rbp[rb];
    box->bneighbors[rb] = rbp[26+rb];
  }
  ocrDbDestroy(depv[32].guid);
  ...
  for(b=0; b<4; ++b) {
    real_t rx = (i+basis[b][0])*sim->pot.lat;
    real_t ry = (j+basis[b][1])*sim->pot.lat;
    real_t rz = (k+basis[b][2])*sim->pot.lat;
    box->gid[rpf->atoms] = b+((k+lattice[2]*(j+lattice[1]*i))<<2);
    u64 seed = mk_seed(box->gid[rpf->atoms], 457);
    rpf->r[rpf->atoms][0] = rx+(2.0*lcg61(&seed)-1.0)*delta;
    rpf->r[rpf->atoms][1] = ry+(2.0*lcg61(&seed)-1.0)*delta;
    rpf->r[rpf->atoms][2] = rz+(2.0*lcg61(&seed)-1.0)*delta;
    if (rx < box->min[0] || rx >= box->max[0]) continue;
    if (ry < box->min[1] || ry >= box->max[1]) continue;
    if (rz < box->min[2] || rz >= box->max[2]) continue;

    rpf->s[rpf->atoms] = 0;
    real_t ms = mass->mass[rpf->s[rpf->atoms]]*sqrt(kB_eV*temperature/mass->mass[rpf->s[rpf->atoms]]);
    seed = mk_seed(box->gid[rpf->atoms], 123);
    vcm_p[0] += rpf->p[rpf->atoms][0] = ms*gasdev(&seed);
    vcm_p[1] += rpf->p[rpf->atoms][1] = ms*gasdev(&seed);
    vcm_p[2] += rpf->p[rpf->atoms][2] = ms*gasdev(&seed);
    vcm_p[3] += mass->mass[rpf->s[rpf->atoms]];
    ++rpf->atoms;
  }

  *atoms = box->max_occupancy = rpf->atoms;
  ...
  ocrAddDependence(depv[4].guid, edt, 1, DB_MODE_RW);
  ocrAddDependence(depv[31].guid, edt, 2, DB_MODE_RO);
  ...
  ocrAddDependence(reduction->reduction, edt, 0, DB_MODE_RO);
  ocrAddDependence(depv[4].guid, edt, 1, DB_MODE_RW);
  ocrAddDependence(depv[1].guid, edt, 2, DB_MODE_RO);
  ocrEdtTemplateDestroy(tmp);

  if(sim->pot.potential&LJ) {
    pv[0] = *(u64*)&sim->pot.lj.sigma; pv[1] = *(u64*)&sim->pot.lj.epsilon;
    pv[2] = *(u64*)&sim->pot.cutoff; pv[3] = *(u64*)&sim->boxes.domain[0];
    pv[4] = *(u64*)&sim->boxes.domain[1]; pv[5] = *(u64*)&sim->boxes.domain[2];

    ocrGuid_t tfin=fin;
    ocrEdtTemplateCreate(&tmp,sim->pot.force_edt,6,27);
...
ocrGuid_t fork_init_edt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
  ...
  ocrGuid_t* box = (ocrGuid_t*)depv[1].ptr;
  ocrGuid_t* rpf = (ocrGuid_t*)depv[2].ptr;

  ocrGuid_t sch_g, *sch_p;
  ocrDbCreate(&sch_g, (void**)&sch_p, sizeof(ocrGuid_t)*sim->boxes.boxes_num, DB_PROP_NONE, NULL_GUID, NO_ALLOC);
  u32 b; void* p;
  for(b=0; b < sim->boxes.boxes_num; ++b)
    ocrDbCreate(sch_p+b, &p, sizeof(ocrGuid_t)*26, DB_PROP_NO_ACQUIRE, NULL_GUID, NO_ALLOC);

  reduction_t* reduction;
  ocrDbCreate(&sim->reduction, (void**)&reduction, sizeof(reduction_t), DB_PROP_NONE, NULL_GUID, NO_ALLOC);

  ocrGuid_t* leaves_p; ocrGuid_t leaves_g;
  ocrDbCreate(&leaves_g, (void**)&leaves_p, sizeof(ocrGuid_t)*sim->boxes.boxes_num*3, DB_PROP_NONE, NULL_GUID, NO_ALLOC);
  u64 red_paramv[3];
  red_paramv[1] = sim->pot.potential&LJ ? *(u64*)&sim->pot.lj.epsilon : 1;
  reduction->reduction = build_reduction(depv[0].guid, sim->reduction, sim->boxes.boxes_num, leaves_p+sim->boxes.boxes_num*2, 2, red_paramv, ured_edt);
  red_paramv[1] = paramv[4]; paramv[2] = reduction->reduction;
  ...
  for(ijk[2]=0; ijk[2]<grid[2]; ++ijk[2]) {
    ((u64*)ijk)[5] = leaves_p[leaf];
    ((u64*)ijk)[6] = leaves_p[leaf+sim->boxes.boxes_num];
    ((u64*)ijk)[7] = leaves_p[leaf+sim->boxes.boxes_num*2];
    ocrEdtCreate(&edt, tmp, 8, (u64*)ijk, 33, NULL, EDT_PROP_NONE, NULL_GUID, NULL);
    ocrAddDependence(depv[0].guid, edt, 0, DB_MODE_RO);
    ocrAddDependence(sim->pot.mass, edt, 1, DB_MODE_RW);
    ocrAddDependence(sim->reduction, edt, 2, DB_MODE_RW);
    ocrAddDependence(box[leaf], edt, 3, DB_MODE_RW);
    ocrAddDependence(rpf[leaf], edt, 4, DB_MODE_RW);
    ...
    for(b=0; b < 26; ++b) {
      ...
      ocrAddDependence(sch_p[n], edt, 5+b, DB_MODE_RW);
      rbp[b] = rpf[n]; rbp[26+b] = box[n];
    }
    ocrAddDependence(sch_p[leaf], edt, 31, DB_MODE_RO);
    ocrAddDependence(rb, edt, 32, DB_MODE_RO);
    ++leaf;
  }
  ocrEdtTemplateDestroy(tmp);
