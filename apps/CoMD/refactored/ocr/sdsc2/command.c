...
static void add_arg(const char* long_option, const char short_option,
                    ...
                    const char* help, command_t* cmd)
{
  option_t* o = cmd->options+cmd->options_num;
  strcpy(o->help, help);
...
static void process_args(int argc, char** argv, command_t* cmd)
{
  while(1) {
    int option_index = 0;
...
void parse_command(int argc, char** argv, command_t* cmd)
{
  ...
  add_arg("delta",    'r', 1, 'd', &(cmd->delta),         0,             "initial delta (Angstroms)", cmd);
  ...
  if(help)
    ...
    //       "  ny: %d\n"
    //       "  nz: %d\n"
    //       "  Lattice constant: %.3f Angstroms\n"
    //       "  Steps: %u\n"
    //       "  Period: %u\n"
    //       "  Time step: %.3f fs\n"
    //       "  Initial Temperature: %.3f K\n"
    //       "  Initial Delta: %.3f Angstroms\n"
    //       "\n",
    //       cmd->doeam,
    //       cmd->pot_dir,
    //       cmd->pot_name,
