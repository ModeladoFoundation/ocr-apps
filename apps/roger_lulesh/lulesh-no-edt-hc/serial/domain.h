// use this instead of the naked 8 constant
...
/* manner. */
   ...
   void domain_AllocateNodalPersistent(size_t hcSize)
   {
      ...
      if(domain.m_xd != NULL)free(domain.m_xd); domain.m_xd = malloc(hcSize*sizeof(Real_t));for(Index_t i=0; i<hcSize; ++i) domain.m_xd[i] = (Real_t)(0.0);
      if(domain.m_yd != NULL)free(domain.m_yd); domain.m_yd = malloc(hcSize*sizeof(Real_t));for(Index_t i=0; i<hcSize; ++i) domain.m_yd[i] = (Real_t)(0.0);
      if(domain.m_zd != NULL)free(domain.m_zd); domain.m_zd = malloc(hcSize*sizeof(Real_t));for(Index_t i=0; i<hcSize; ++i) domain.m_zd[i] = (Real_t)(0.0);

      if(domain.m_xdd != NULL)free(domain.m_xdd); domain.m_xdd = malloc(hcSize*sizeof(Real_t));for(Index_t i=0; i<hcSize; ++i) domain.m_xdd[i] = (Real_t)(0.0);
      if(domain.m_ydd != NULL)free(domain.m_ydd); domain.m_ydd = malloc(hcSize*sizeof(Real_t));for(Index_t i=0; i<hcSize; ++i) domain.m_ydd[i] = (Real_t)(0.0);
      if(domain.m_zdd != NULL)free(domain.m_zdd); domain.m_zdd = malloc(hcSize*sizeof(Real_t));for(Index_t i=0; i<hcSize; ++i) domain.m_zdd[i] = (Real_t)(0.0) ;
      ...
      if(domain.m_nodalMass != NULL)free(domain.m_nodalMass); domain.m_nodalMass = malloc(hcSize*sizeof(Real_t));for(Index_t i=0; i<hcSize; ++i) domain.m_nodalMass[i] = (Real_t)(0.0) ;
   }
   ...
   void domain_AllocateElemPersistent(size_t hcSize)
   {
      ...
      if(domain.m_e != NULL)free(domain.m_e); domain.m_e = malloc(hcSize*sizeof(Real_t));for(Index_t i=0; i<hcSize; ++i) domain.m_e[i] = (Real_t)(0.0);

      if(domain.m_p != NULL)free(domain.m_p); domain.m_p = malloc(hcSize*sizeof(Real_t));for(Index_t i=0; i<hcSize; ++i) domain.m_p[i] = (Real_t)(0.0);
      ...
      if(domain.m_v != NULL)free(domain.m_v); domain.m_v = malloc(hcSize*sizeof(Real_t));for(Index_t i=0; i<hcSize; ++i) domain.m_v[i] = (Real_t)(1.0);
