...
#include "../../tools/rmd_afl/rmd_memory/rmd_memory.h"
#include "../../tools/rmd_afl/rmd_memory/rmd_memory_pthrhack.h"
#include "../../tools/rmd_afl/rmd_afl_all.h"

void aflmain();
static int rmdinit=0;
...
int main(int argc, char** argv) {
  ...
  int rob;
  MPI_Comm_size(MPI_COMM_WORLD, &rob);
  printf("num ranks = %d\n",rob);
...
#define MB (1024 * KB)
#define GB (1024 * MB)
#define DRAM_SIZE       (1024*MB)
#define NUM_DRAM        (1)
#define BLOCK_MEM_SIZE  (1024*KB)
#define NUM_BLOCK_MEM   (2)
#define LOCAL_MEM_SIZE  (1024*KB)
#define NUM_LOCAL_MEM   (1)
#endif


void aflmain() {

  // perhaps later put in config file reading, etc.
  ...
  printf("processor %s, rank %d (of %d) initializing afl\n",
  ...
  //  }
  agentid_t myid;
  myid = afl_gen_core_id(0,0,myrank);
  printf("generated coreid %d for rank%d\n",afl_get_core_id(myid),myrank);
