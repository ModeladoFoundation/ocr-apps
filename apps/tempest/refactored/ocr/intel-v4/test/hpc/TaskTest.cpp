...
ocrGuid_t updatePatch(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    if (step ==0) {
            ...
            ebr.GetExchangeBuffersBySourcePatchIx(rank, vecExchangeBufferIndices);
            if (vecExchangeBufferIndices.size() > ParamNeighborsPerPatch) {
                 _EXCEPTION1("Insufficient ParamNeighborsPerPatch (>= %lu)",  vecExchangeBufferIndices.size());
            } else {
                PRINTF("GJDEBUG1: Patch = %d, NumNeighborPatches = %d\n", rank, vecExchangeBufferIndices.size());
            }
            fflush (stdout);
    }
...
try {
	...
	// Set the model grid (one patch Cartesian grid)
		...
		new GridCartesianGLL(
			...
			dRefLat,
			eVerticalStaggering);
                model.SetGrid(pGrid [i], ParamTotalPatchCount, false);
	...
	// Build the DataContainer object for the GridPatch objects
        ...
        for (int i = 0; i<nWorkers; i++) {
            PRINTF("GJDEBUG: setting up patch %d\n", i);
            fflush(stdout);
	    ...
	    std::cout << "Allocating data ... " << std::endl;
            ...
            ExchangeBufferRegistry & ebr = pGrid [i]->GetExchangeBufferRegistry();
            std::vector<int> vecExchangeBufferIndices;
            ebr.GetExchangeBuffersBySourcePatchIx(i, vecExchangeBufferIndices);
            if (vecExchangeBufferIndices.size() > ParamNeighborsPerPatch) {
                 _EXCEPTION1("Insufficient ParamNeighborsPerPatch (>= %lu)",  vecExchangeBufferIndices.size());
            } else {
                PRINTF("GJDEBUG1: Patch = %d, NumNeighborPatches = %d\n", i, vecExchangeBufferIndices.size());
                fflush (stdout);
            }
            fflush (stdout);
