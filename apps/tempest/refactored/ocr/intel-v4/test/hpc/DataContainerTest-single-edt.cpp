...
extern "C" ocrGuid_t mainEdt ( u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    if (argc != 2) {
        ...
        u32 i = 1;
        num_workers = (u32) atoi(getArgv(programArgv, i++));
    }
...
try {
	...
	// Model parameters
	static const int nResolutionX = 10;
	static const int nResolutionY = 10;
	static const int nHorizontalOrder = 4;
	static const int nVerticalOrder = 4;
	...
	//MPI_Finalize();
        ocrShutdown ();
        PRINTF ("Good-by from DataContainerTest EDT");
}
