...
void Begin(beginParams &params, ocxxr::Datablock<ocxxr::BasedPtr<Model>> bp, ocxxr::Arena<void> arena)
{
...
try {
	...
	EndTempestCommandLine(args)

    beginParams bParams = {dU0, dH0, dAlpha};

	// Setup the Model
	AnnounceBanner("MODEL SETUP");

    auto arena = ocxxr::Arena<void>::Create( DEFAULT_ARENA_SIZE );
    auto bp = ocxxr::Datablock<ocxxr::BasedPtr<Model>>::Create();

    EquationSet eqn = EquationSet::ShallowWaterEquations;

    Model * model = arena.New<Model>(eqn);
    *bp = model;

	TempestSetupCubedSphereModel(*model, arena);

    auto begin_template = OCXXR_TEMPLATE_FOR( Begin );
	AnnounceStartBlock("Initializing test case");
	model->SetTestCase(
		arena.New<ShallowWaterTestCase2>(dH0, dU0, dAlpha));
    ...
    bp.Release();
    arena.Release();
    begin_template().CreateTask(bParams, bp, arena);
