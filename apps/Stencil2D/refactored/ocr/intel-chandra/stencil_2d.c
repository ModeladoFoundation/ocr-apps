...
_OCR_TASK_FNC_( FNC_globalCompute );
    _OCR_TASK_FNC_( FNC_globalCompute_setUp );
...
ocrGuid_t FNC_init_globalParamH(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
    ...
    int Num_procs = PTR_globalParamH->NR;
...
ocrGuid_t FNC_init_globalH(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
    ...
    ocrEdtCreate( &TS_init_globalH_part1.EDT, TS_init_globalH_part1.TML,
                  EDT_PARAM_DEF, NULL, EDT_PARAM_DEF, NULL,
                  EDT_PROP_NONE, NULL_GUID, &TS_init_globalH_part1.OET );
...
ocrGuid_t FNC_init_globalH_part1(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
    ...
    PTR_globalH->itimestep = 1; //Initial timestep is 1
...
ocrGuid_t FNC_rankInitSpawner(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
    ...
    for( i = 0; i < NR; i++ )
    {
        ...
        ocrAddDependence( PTR_globalH->DBK_EVT_IN_reduction, TS_rankInit.EDT, _idep++, DB_MODE_CONST );
...
ocrGuid_t FNC_rankInit(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
    ...
    ocrAddDependence( DBK_EVT_IN_reduction, TS_init_rankH.EDT, _idep++, DB_MODE_CONST );
...
ocrGuid_t FNC_init_rankH(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
    ...
    ocrGuid_t DBK_rankParamH = depv[2].guid;

    globalParamH_t *PTR_globalParamH = (globalParamH_t*) depv[0].ptr;
    ocrGuid_t* PTR_EVT_IN_reduction = depv[1].ptr;
    rankParamH_t *PTR_rankParamH = (rankParamH_t*) depv[2].ptr;
    rankDataH_t *PTR_rankDataH = (rankDataH_t*) depv[3].ptr;
    ...
    PTR_rankParamH->itimestep = 1; //Initial timestep is 1
    ...
    for( i = 0; i < 2; i++ )
    {
        rankEventH_t *PTR_rankEventH = (rankEventH_t*) depv[4+i].ptr;
        ...
        PTR_rankEventH->EVT_IN_reduction = PTR_EVT_IN_reduction[id];
    }

    rankTemplateH_t *PTR_rankTemplateH = depv[6].ptr;
...
ocrGuid_t FNC_globalCompute(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
    ...
    TS_globalCompute_setUp.FNC = FNC_globalCompute_setUp;
    ocrEdtTemplateCreate( &TS_globalCompute_setUp.TML, TS_globalCompute_setUp.FNC, _paramc, _depc );

    ocrEdtCreate( &TS_globalCompute_setUp.EDT, TS_globalCompute_setUp.TML,
                  EDT_PARAM_DEF, NULL, EDT_PARAM_DEF, NULL,
                  EDT_PROP_NONE, NULL_GUID, &TS_globalCompute_setUp.OET );

    ocrAddDependence( TS_globalCompute_setUp.OET, TS_globalCompute_setUp_OET, 0, DB_MODE_NULL );
    ...
    MyOcrTaskStruct_t TS_rankComputeSpawner; _paramc = 1; _depc = 4;
    ...
    ocrEdtCreate( &TS_rankComputeSpawner.EDT, TS_rankComputeSpawner.TML,
                  ...
                  EDT_PROP_FINISH, NULL_GUID, &TS_rankComputeSpawner.OET );
    ...
    ocrAddDependence( TS_globalCompute_setUp_OET, TS_rankComputeSpawner.EDT, _idep++, DB_MODE_NULL );
    ...
    ocrAddDependence( PTR_globalH->DBK_timers, TS_summary.EDT, _idep++, DB_MODE_RW );
    ocrAddDependence( PTR_globalH->EVT_OUT_reduction, TS_summary.EDT, _idep++, DB_MODE_RW );
...
//( )
//{ DBK_globalParamH <COST>, DBK_globalH <RW>, DBK_timers <RW>, DBK_EVT_IN_reduction <COST> }
ocrGuid_t FNC_globalCompute_setUp(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
    //PRINTF("%s\n", __func__);
    u32 _paramc, _depc, _idep;
...
ocrGuid_t FNC_rankComputeSpawner(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
    ...
    for( I = 0; I < NR; I++ )
    {
        ...
        ocrEdtCreate( &TS_rankMultiTimestepper.EDT, TS_rankMultiTimestepper.TML,
                      ...
                      EDT_PROP_FINISH, currentAffinity, NULL );
...
//( )
//{ DBK_globalParamH<CONST>, DBK_timers<RW>, EVT_OUT_reduction <RW>, EVT_control_dep<NULL> }
