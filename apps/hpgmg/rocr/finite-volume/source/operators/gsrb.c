...
//------------------------------------------------------------------------------------------------------------------------------
	...
	#include "gsrb_mapped_black.c"
    #include "gsrb_mapped_red.c"
#endif
      ...
      const double * __restrict__ RedBlack[2] = {level->RedBlack_FP[0] + ghosts*(1+jStride),
        ...
        int ghostsToOperateOn=ghosts-1;
            ...
            for ( ss=s;ss<s+ghosts;ss++,ghostsToOperateOn--) {
                if ( (ss^color000^1)&1 ) {
			        GSRB_R_STREAM_EXEC(black,6) // report error if variant not found and exit
                } else {
                    GSRB_R_STREAM_EXEC(red,6) // report error if variant not found and exit
                }
            }
		#else
