...
ocrGuid_t test_box_edt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
  ...
  for (i=1;i<depc;i++) {
    ...
    vol = l->volume;
    dim = l->box_dim;
    ocrPrintf("level->h: %f, Boxid: %u, volume: %u, dim: %u\n", l->h,boxid, vol, dim);
  }
  return NULL_GUID;
}
