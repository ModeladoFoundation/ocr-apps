...
fftwf_plan fftwf_plan_dft_1d(int size, fftwf_complex *in, fftwf_complex *out, int dir, int est)
{
	...
	plan.out  = out;
#ifdef TRACE
  fprintf(stderr,"PLAN dir(%2d) Pof2(%d) size(%d) SIZE(%d) in(%p) out(%p)\n",plan.dir,plan.Pof2,plan.size,plan.SIZE,plan.in,plan.out);fflush(stderr);
#endif
...
#ifdef DEBUG
  printf("EXECUTE dir(%2d) Pof2(%d) size(%d) SIZE(%d) in(%p) out(%p)\n",dir,Pof2,size,SIZE,z_in,z_out);
#endif
