...
fprintf(stderr,"process FileType number %d\n",wantFileType);fflush(stderr);
	...
	if( !(ts_params.Scf % 2) ) {
		printf("Scf must be odd. Exiting.\n");
		exit(1);
	}
	...
	image_params.S2 = image_params.S1/image_params.TF;
        ...
        if( wantFileType == 1 ) { // Data.bin -> Data.s
...
fprintf(stderr,"write FreqDomain (ascii)\n");fflush(stderr);
	printf("	.file	\"Data.s\"\n");
	printf("	.type	image_%d,@object\n",image);
	printf("	.data\n");
	printf("	.globl	image_%d\n",image);
	printf("	.size	image_%d, %d\n",image,2*image_params.P1*image_params.S1*sizeof(uint32_t));
	printf("	.align	16\n");
	printf("image_%d:\n",image);
		for(int iy=0;iy<image_params.P1;iy++) {
			for(int ix=0;ix<image_params.S1;ix++) {
	        		printf(".d4bu %u\n.d4bu %u\n",
					*(int32_t *)&FreqDomain[iy][ix].real,
					*(int32_t *)&FreqDomain[iy][ix].imag);
		        }
		}
	...
	else if ( wantFileType == 2 ) { // PlatformPostion.bin -> PlatformPostion.s
	    for(int image = 0; image < image_params.numImages; image++) {
...
fprintf(stderr,"write PlatformPosition (ascii)\n");fflush(stderr);
	printf("	.file	\"PlatformPosition.s\"\n");
	printf("	.type	platform_%d,@object\n",image);
	printf("	.data\n");
	printf("	.globl	platform_%d\n",image);
	printf("	.size	platform_%d, %d\n",image,3*image_params.P1*sizeof(uint32_t));
	printf("	.align	16\n");
	printf("platform_%d:\n",image);
		for(int iy=0;iy<image_params.P1;iy++) {
	...
	fprintf(stderr,"free PlatformPosition\n");fflush(stderr);
        ...
        else if( wantFileType == 3 ) {  // PulseTransmissionTime.bin -> PulseTransmissionTime.s
	    for(int image = 0; image < image_params.numImages; image++) {
...
fprintf(stderr,"write PulseTransmission (ascii)\n");fflush(stderr);
	printf("	.file	\"PulseTransmissionTime.s\"\n");
	printf("	.type	pulse_%d,@object\n",image);
	printf("	.data\n");
	printf("	.globl	pulse_%d\n",image);
	printf("	.size	pulse_%d, %d\n",image,1*image_params.P1*sizeof(uint32_t));
	printf("	.align	16\n");
	printf("pulse_%d:\n",image);
		for(int iy=0;iy<image_params.P1;iy++) {
			for(int ix=0;ix<1;ix++) {
