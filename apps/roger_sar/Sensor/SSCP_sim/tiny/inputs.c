...
#ifdef RAG_IMPLICIT_INPUTS
  extern rmdglobal struct {int32_t real; int32_t imag;} image_0[RAG_P1][RAG_S1];
  extern rmdglobal struct {int32_t real; int32_t imag;} image_1[RAG_P1][RAG_S1];
  extern rmdglobal struct {int32_t x; int32_t y; int32_t z;} platform_0[RAG_P1][1];
  extern rmdglobal struct {int32_t x; int32_t y; int32_t z;} platform_1[RAG_P1][1];
  extern rmdglobal struct {int32_t t; } pulse_0[RAG_P1][1];
  extern rmdglobal struct {int32_t t; } pulse_1[RAG_P1][1];
#ifdef TRACE_LVL_2
...
#endif
#ifdef DEBUG
  xe_printf("//// first image (0x%x, 0x%x)\n",
RAG_GET_UINT(((uint32_t *)RAG_GET_PTR(in->X))+0),
RAG_GET_UINT(((uint32_t *)RAG_GET_PTR(in->X))+1));
  xe_printf("//// first platform position (0x%x, 0x%x, 0x%x)\n",
RAG_GET_UINT(((uint32_t *)RAG_GET_PTR(in->Pt))+0),
RAG_GET_UINT(((uint32_t *)RAG_GET_PTR(in->Pt))+1),
RAG_GET_UINT(((uint32_t *)RAG_GET_PTR(in->Pt))+2));
  xe_printf("//// first pulse transmission timestamp (0x%x)\n",
RAG_GET_UINT(((uint32_t *)in->Tp)+0));
#endif
#ifdef RAG_IMPLICIT_INPUTS
