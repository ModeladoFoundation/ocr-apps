...
#ifdef DEBUG
  xe_printf("//// FIRST image at (%d,%d) = (0x%x, 0x%x)\n", n1, n1,
RAG_GET_UINT(((uint32_t *)((struct complexData *)RAG_GET_PTR(image+m1))+n1)+0),
RAG_GET_UINT(((uint32_t *)((struct complexData *)RAG_GET_PTR(image+m1))+n1)+1));
#endif
      free(image_data_ptr);
      free(image_ptr);
#endif
