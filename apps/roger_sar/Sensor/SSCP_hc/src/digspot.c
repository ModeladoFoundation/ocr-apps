...
#ifdef DEBUG
fprintf(stderr,"DigSpot P1, S1 <%3d,%3d>\n",image_params->P1, image_params->S1);
fprintf(stderr,"DigSpot P2, S2 <%3d,%3d>\n",image_params->P2, image_params->S2);
fprintf(stderr,"DigSpot P3, S3 <%3d,%3d>\n",image_params->P3, image_params->S3);
fprintf(stderr,"freqVec %p\n",dig_spot->freqVec);
fflush(stderr);
#endif
	ifft_input  = (fftwf_complex*)fftwf_malloc(image_params->S1 * sizeof(fftwf_complex));
	ifft_result = (fftwf_complex*)fftwf_malloc(image_params->S1 * sizeof(fftwf_complex));
	plan_backward = fftwf_plan_dft_1d(image_params->S1, ifft_input, ifft_result, FFTW_BACKWARD, FFTW_ESTIMATE);

	fprintf(stderr,"Xc = %7.2fm Yc = %7.2fm\n", xc, yc);fflush(stderr);
