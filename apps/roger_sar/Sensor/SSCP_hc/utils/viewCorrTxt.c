...
fprintf(stderr,"malloc corr_map\n");fflush(stderr);
	corr_map = (struct hcPoint **)malloc(Cy*sizeof(struct hcPoint *));
	if(!corr_map) {perror("malloc of corr_map[]");exit(1);}
	for(int iy=0;iy<Cy;iy++) {
		corr_map[iy] = (struct hcPoint *)malloc(Cx*sizeof(struct hcPoint));
...
fprintf(stderr,"read corr_map\n");fflush(stderr);
	for(int iy=0;iy<Cy;iy++) {
#if 0
		fread(&corr_map[iy][0],sizeof(struct hcPoint),Cx,corr_debug);
#else
		for(int ix=0;ix<Cx;ix++) {
			if( 3 != fscanf(corr_debug,"CONSOLE: XE7 >>> corr %x %x %x\n",
