...
/*
 * This is implementation specific for the tgr runtime. It is never used by newlib
 * execpt when it needs to allocate space for it. The main thread will have the memory
 * for this managed by itself by the runtime, but subthreads need the pointer to this
 * struct in the reent struct to be initialized. The pointer must point to a zero'ed
 * chunck of memory which is tgr_sizeof_reent in size.
 */
struct _tgr_reent;
