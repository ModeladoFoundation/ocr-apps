...
* TG runtime integration
    tg runtime provides all memory (Data Blocks) via the tgr_mmap() API.
    ...
    The USE_TGR define is used instead of USE_MMAP, but USE_ARENAS is still used.
    Each TGR allocated chunck of memory has an initial struct containing its GUID
    and a pointer forming a linked list to allow all allocated DB destruction on exit.
    ...
    NOTE: This file is derived from the libc/sys/linux/malloc.c version instead
          ...
          assumed that the TGR calls manage the cache as necessary.
...
#if USE_TGR
//
// Setup constants required to compile for TGR use
//
#include <sys/tgr.h>
...
#ifdef USE_TGR
/* This is at the head of every malloc created tgr chuck of memory
*/
typedef struct _tgr_info {
    uint64_t len;
    struct _tgr_info * fd;
} tgr_info;

/* All allocated chunks of memroy are put on this list */
static tgr_info * tgr_info_list = NULL;

/* This is the equivalent to an mmapped chunk but with an integral structure
   to help track TGR allocated chunks. We keep a list of them to allow for deletion
   at exit */
typedef struct _tgr_chunk {
    tgr_info  info;
    malloc_chunk chunk;
} tgr_chunk;
...
/* When using TGR, all arenas are contained in a heap and dynamically allocated.
*/
...
#if USE_TGR
/* Heaps are created with tgr_mmap() and allocated for 2 reasons:
 * - to create a new arena, including initial allocatable space.
 * - to create an additional memory region for arena allocation.
 * All heaps have an initial TGR info structure like tgr_chunks that
 * is used to track allocated addresses and sizes
 */
typedef struct _tgr_heap {
    tgr_info info;
    heap_info   hinfo;
} tgr_heap;
...
#ifdef USE_TGR
/* In TGR use, all arenas are heaps, no static main_arena. This saves
 * the 2K+ bytes of local scratch pad on TGR at the expense of slightly
...
__tgr_alloc(size_t size)
{
   ...
   */
  size = (size + sizeof(tgr_info) + page_mask) & ~page_mask;

  ret = tgr_mmap((void *)&info, size);
  if(ret != 0)
    return NULL;

  info->len = size;
  info->fd = tgr_info_list;
  tgr_info_list = info;
...
static void
internal_function
__tgr_free( tgr_info * this_info )
{
  /* Find and remove from the db chain */
  for( tgr_info ** opp = &tgr_info_list; *opp ; opp = &((*opp)->fd) ) {
    if( *opp == this_info ) {
        *opp = this_info->fd;
        break;
    }
  }
  int ret = tgr_munmap( (void*)this_info, this_info->len );

  /* tgr_munmap returns non-zero on failure */
  assert(ret == 0);
}
...
mmap_chunk(size_t size)
{
  ...
  tgr_chunk *c = (tgr_chunk *)__tgr_alloc( size );
 ...
 */
#define chunk2tgr_chunk(p) \
        ((tgr_chunk *)((char *)(p) - (p)->prev_size - offsetof(tgr_chunk,chunk)))
...
munmap_chunk(mchunkptr p)
{
  ...
  tgr_chunk *c = chunk2tgr_chunk(p);

  __tgr_free( & c->info );
}

#define heap2tgr_heap(h) \
        ((tgr_heap *)((char *)(h) - offsetof(tgr_heap,hinfo)))
//
// Create a new heap
...
new_heap(size_t size)
{
  ...
  tgr_heap *oh = (tgr_heap *)__tgr_alloc( size );
  if( oh == NULL )
    return NULL;
