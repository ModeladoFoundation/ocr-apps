...
* OCR integration
    OCR provides all memory (Data Blocks) via the ocrDBCreate() API.
    This method is used in place of sbrk() and mmap() to allocate heaps and
    large chunks.
    The main_arena variable is renamed 'arenas' and changed to be a pointer to
    the first allocated arena instead of a staticly allocated structure.
    The USE_OCR define is used instead of USE_MMAP, but USE_ARENAS is still used.
    Each OCR allocated DB has an initial struct containing its GUID and a pointer
    forming a linked list to allow all allocated DB destruction on exit.

    NOTE: This file is derived from the libc/sys/linux/malloc.c version instead
          of the libc/stdlib/mallocr.c. Although the stdlib version supports mmap
          for large blocks, unlike the linux version, it doesn't support mmapped
          heaps/arenas or pthread-based thread arena affinity which may be useful
          when extending to multiple XE operation.
          Also, there is no cache management done in this file. It is currently
          assumed that the OCR calls manage the cache as necessary.
...
#if USE_OCR
//
// Setup constants required to compile for OCR use
//
#include "ocr.h"
...
#define NO_THREADS      1
//
// we don't have pages as such, but we need to guarantee a minimum
// alignment. This is the smallest alignment necessary for chunks
//
#define malloc_getpagesize MINHEADSIZE   // min chunk size
...
/* Should be malloc.h, but for now ...
 */
struct mallinfo
{
...
#endif
#if USE_ARENAS && !(HAVE_MMAP || USE_OCR)
...
typedef struct _malloc_chunk
{
  ...
  INTERNAL_SIZE_T size;      /* Size in bytes, including overhead - 2*SIZE_T */
...
#ifdef USE_OCR
/* This is at the head of every malloc created OCR db
*/
typedef struct _ocr_db_info {
    ocrGuid_t guid;
    struct _ocr_db_info * fd;
} ocr_db_info;

/* All allocated dbs are put on this list */
static ocr_db_info * ocr_db_list = NULL;

/* This is the equivalent to an mmapped chunk but with an integral structure
   to help track OCR allocated chunks. We keep a list of them to allow for deletion
   at exit */
typedef struct _ocr_chunk {
    ocr_db_info  info;
    malloc_chunk chunk;
} ocr_chunk;
...
/* When using OCR, all arenas are contained in a heap and dynamically allocated.
*/
...
typedef struct _heap_info {
  ...
  size_t size;      /* Current size in bytes including arena and containing heap overhead */
  size_t offset;    /* Start of chunks (after heap_info or arena, depending) */
} heap_info;

#if USE_OCR
/* Heaps are created with ocrDbCreate() and allocated for 2 reasons:
 * - to create a new arena, including initial allocatable space.
 * - to create an additional memory region for arena allocation.
 * All heaps have an initial OCR info structure like ocr_chunks that
 * is used to track allocated GUIDs.
 */
typedef struct _ocr_heap {
    ocr_db_info info;
    heap_info   hinfo;
} ocr_heap;

/* Overlay on a heap allocated by arena_get2() when creating a new arena
 */
typedef struct _arena_heap {
    heap_info  hinfo;
    arena      arena;
} arena_heap;
...
#define MALLOC_ALIGN(p)        (((char *)(p) + MALLOC_ALIGNMENT) & ~(MALLOC_ALIGNMENT - 1))

#define MINSIZE                (sizeof(malloc_chunk))       // the whole chunk header
#define MINHEADSIZE            (offsetof(malloc_chunk,fd))  // just the size parts

/* conversion from malloc headers to user pointers, and back */

// #define chunk2mem(p) ((Void_t*)((char*)(p) + 2*SIZE_SZ))
// #define mem2chunk(mem) chunk_at_offset((mem), -2*SIZE_SZ)
#define chunk2mem(p) (Void_t*)((char*)(p) + MINHEADSIZE)
#define mem2chunk(mem) chunk_at_offset((mem), -MINHEADSIZE)
...
#define _bin_at(a, i)  ((mbinptr)((char*)&(((a)->av)[2*(i)+2]) - offsetof(malloc_chunk,fd)))
...
#ifdef USE_OCR
/* In OCR use, all arenas are heaps, no static main_arena. This saves
 * the 2K+ bytes of local scratch pad on XSTG at the expense of slightly
 * slower access to the main arena structure.
 */
...
static void
internal_function
ocr_free( ocr_db_info * this_info )
{
  /* Find and remove from the db chain */
  for( ocr_db_info ** opp = &ocr_db_list; *opp ; opp = &((*opp)->fd) ) {
    if( *opp == this_info ) {
        *opp = this_info->fd;
        break;
    }
  }
  int ret = ocrDbDestroy( this_info->guid );

  /* ocrDbDestroy returns non-zero on failure */
  assert(ret == 0);
}

/* Return a newly allocated chunk of memory.
 * size already includes the malloc_chunk overhead.
 */
static mchunkptr
internal_function
mmap_chunk(size_t size)
{
  dprintf("mmap_chunk 0x%08lx\n", size );
  ...
  ocr_chunk *c = (ocr_chunk *)ocr_alloc( size );
...
#ifdef NO_THREADS
  if ((unsigned long)(mmapped_mem + arena_mem) > max_total_mem)
    max_total_mem = mmapped_mem + arena_mem + sbrked_mem;
#endif
  return p;
}

/* This should only be called for 'mmapped' chunks
 * We subtract prev_size because this chunk may have been realigned.
 */
#define chunk2ocr_chunk(p) \
        ((ocr_chunk *)((char *)(p) - (p)->prev_size - offsetof(ocr_chunk,chunk)))

static void
internal_function
munmap_chunk(mchunkptr p)
{
  dprintf("munmap_chunk 0x%016p\n", p );
  ...
  ocr_chunk *c = chunk2ocr_chunk(p);

  ocr_free( & c->info );
}

#define heap2ocr_heap(h) \
        ((ocr_heap *)((char *)(h) - offsetof(ocr_heap,hinfo)))
//
// Create a new heap
// Heaps are maintained in per-arena lists off of the arena's
// heap prev pointer.
//
static heap_info *
internal_function
...
{
  dprintf("new_heap 0x%08lx\n", size );
  size_t page_mask = malloc_getpagesize - 1;

  size += sizeof(heap_info);
  ...
  ocr_heap *oh = (ocr_heap *)ocr_alloc( size );
  if( oh == NULL )
    return NULL;

  heap_info *h = &(oh->hinfo);

  h->size = size;   /* include overhead */
  h->ar_ptr = NULL;
  h->prev = NULL;   /* Previous heap. */
  h->offset = sizeof(*h);
...
static void
internal_function
delete_heap(heap_info *h)
{
  dprintf("delete_heap %p\n", h );
  //
  // Take us off the arena heap list
  //
  heap_info *ph;

  for( ph = h->prev ; ph->prev != h ; ph = ph->prev )
    ;
  ph->prev = h->prev;
  //
  // make it go away
  //
  ocr_heap * oh = heap2ocr_heap(h);
  ocr_free( & oh->info );
...
arena_get2(arena *a_tsd, size_t size)
{
  ...
  dprintf("arena_get2 - new heap 0x%08lx\n", size );
  /* create a new arena */
  arena_heap *ah = (arena_heap *)new_heap(size + (sizeof(*a)));
  if(!ah) {
    /* Maybe size is too large to fit in a single heap.  So, just try
       to create a minimally-sized arena and let chunk_alloc() attempt
       to deal with the large request via mmap_chunk(). */
    ah = (arena_heap *)new_heap(sizeof(*a) + MALLOC_ALIGNMENT);
    if(!ah)
      return 0;
  }
  a = &ah->arena;
  ah->hinfo.ar_ptr = a;
  ah->hinfo.offset = sizeof(*ah);
  //
  // an arena's heap is the head and tail of arena associated heaps
  //
  ah->hinfo.prev = & ah->hinfo;

  for(int i = 0; i < NAV ; i++)
    init_bin(a, i);

  a->next = NULL;
  a->size = ah->hinfo.size;
  arena_mem += a->size;

#ifdef NO_THREADS
  if((unsigned long)(mmapped_mem + arena_mem) > max_total_mem)
    max_total_mem = mmapped_mem + arena_mem;
#else
  tsd_setspecific(arena_key, (Void_t *)a);
  mutex_init(&a->mutex);
  int ret = mutex_lock(&a->mutex); /* remember result */
#endif
  ...
  top(a) = (mchunkptr)ptr;
  /* top chunk size is heap size minus heap and arena overheads
   */
  set_head(top(a), (((char*)ah + a->size) - ptr) | PREV_INUSE);

  /* Add the new arena to the list. */
#ifndef NO_THREADS
  (void)mutex_lock(&list_lock);
#endif
  /* The arenas are in a circular list with 'arenas' pointing to the first
   * allocated
   */
  if( arenas == NULL ) {
    a->next = arenas = a;
  ...
  if(ret) /* locking failed; keep arena for further attempts later */
    return 0;
...
#define arena2heap(a) \
        ((heap_info *)((char *)(a) - offsetof(arena_heap,arena)))

#define ptr_in_heap(ptr,h) \
    ((char *)ptr >= ((char *)(h) + (h)->offset) && \
     (char *)ptr <  ((char *)(h) + (h)->size))

//
// test chunk ownership for each heap assoc with this arena
//
static heap_info *
internal_function
...
//
// Search all arenas for a heap that contains this chunk
//
static heap_info *
internal_function
heap_for_ptr( mchunkptr ptr )
{
    arena * a = arenas;
    do {
        heap_info *h = heap_for_ptr_and_arena(ptr,a);
        if( h != NULL )
            return h;
        a = a->next;
    } while( a != arenas );
...
static arena *
internal_function
arena_for_ptr( mchunkptr ptr )
{
    arena * a = arenas;
    do {
        if( heap_for_ptr_and_arena(ptr,a) != NULL )
            return a;
        a = a->next;
    } while( a != arenas );
    return NULL;
...
malloc_extend_top(arena *ar_ptr, INTERNAL_SIZE_T nb)
{
  ...
  mchunkptr       top = top(ar_ptr);
  INTERNAL_SIZE_T top_size = chunksize( top );

  if(top_size < MINSIZE) /* this should never happen */
    return;

  /* enough space in top? */
  if(MINSIZE + nb <= top_size)
    return;

  /* A new heap must be created.
   */
  heap_info *heap = new_heap(nb + MINSIZE);
  if(!heap)
    return;
  heap_info * arheap = arena2heap(ar_ptr);

  heap->ar_ptr = ar_ptr;
  heap->prev = arheap->prev;
  arheap->prev = heap;       /* arena heap points to list of allocated heaps */
  ...
  set_head(top(ar_ptr), (heap->size - sizeof(*heap)) | PREV_INUSE);
  //
  // deal with the old top
  //
  if( top ) {
    ...
    top_size -= MINSIZE;
    set_head(chunk_at_offset(top, top_size + MINHEADSIZE), 0|PREV_INUSE);

    if( top_size >= MINSIZE ) {
      mchunkptr footer = chunk_at_offset(top, top_size);
      set_head( footer, MINHEADSIZE|PREV_INUSE);
      set_foot( footer, MINHEADSIZE);
      set_head_size(top, top_size);
      chunk_free(ar_ptr, top);
    } else {
      set_head(top, (top_size + MINHEADSIZE)|PREV_INUSE);
      set_foot(top, (top_size + MINHEADSIZE));
    }
  }
...
#if USE_OCR
  heap_info *heap = heap_for_ptr_and_arena(p, ar_ptr);
  assert( heap && heap->ar_ptr == ar_ptr);
  if(p != top(ar_ptr))
...
#if USE_ARENAS && MALLOC_DEBUG > 0
/*
  mallarinfo returns a copy of the current mallinfo for arena N.
  Arenas are numbered 0-X. Returns 0 on success, 1 otherwise
*/

int mallarinfo( uint64_t arenaNo, struct mallinfo *mi )
{
  arena * ar = arenas;
  int i = 0;
...
void mall_stats( struct mallstats *ms )
{
    ms->max_system_size = max_total_mem;
    ms->max_regions = max_n_mmaps;
    ms->max_region_size = max_mmapped_mem;
    ms->heaps = stat_n_heaps;
}
...
#endif
{
  ...
  char *ptr = (char*)(heap + 1);

  if(heap->ar_ptr == (arena*)(ptr))
      ptr += sizeof(arena);

  fprintf(stderr, "** Heap %p, start %p, size 0x%010lx:\n",
            heap, ptr, (long)heap->size);

  p = (mchunkptr)(((unsigned long)ptr + MALLOC_ALIGN_MASK) &
                  ~MALLOC_ALIGN_MASK);
  for(;;) {
    char * in_use = inuse(p) ? "(in use) " : "";
    char * mmapped = chunk_is_mmapped(p) ? "(mmapped) " : "";
    fprintf(stderr, "    chunk %p size 0x%010lx %s%s",
                p, (long)p->size & ~SIZE_BITS, in_use, mmapped);
...
dump_arenas( void )
{
    ...
    if( a ) {
        do {
            ...
            heap_info *h = arena2heap(a);
            do {
                dump_heap(h);
                h = h->prev;
             } while( h != arena2heap(a) );
            a = a->next;
        } while( a != arenas );
    } else
        fprintf(stderr,"No arenas\n");
}

#endif  /* USE_ARENAS && MALLOC_DEBUG > 1 */
