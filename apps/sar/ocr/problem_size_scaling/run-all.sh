#!/bin/sh
set -x
OCR_CONFIG=`pwd`/huge-x86-pthread-x86.cfg OCR_TYPE=x86-pthread-x86 make clean all

cp -f ./Parameter0.txt ./Parameters.txt
RUN_TOOL=time OCR_CONFIG=`pwd`/huge-x86-pthread-x86.cfg OCR_TYPE=x86-pthread-x86 make run
