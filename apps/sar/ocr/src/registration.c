...
ocrGuid_t post_Affine_edt(uint32_t paramc, uint64_t *paramv, uint32_t depc, ocrEdtDep_t *depv) {
	int retval;
#ifdef TRACE_LVL_2
ocrPrintf("//// enter post_Affine_edt\n");RAG_FLUSH;
#endif
	assert(paramc==0);
...
#ifdef TRACE_LVL_2
ocrPrintf("//// leave post_Affine_edt\n");RAG_FLUSH;
#endif
	return NULL_GUID;
}
...
ocrPrintf("////// enter post_affine_async_1_edt\n");RAG_FLUSH;
	...
	if(depc!=9) { ocrPrintf("// RAG // RAG // Need to handle N (affine_params->Nc) == 0 case // RAG // RAG// \n");RAG_FLUSH; }
...
#ifdef TRACE_LVL_3
ocrPrintf("////// disp_vec.p >= affine_params->Tc\n");RAG_FLUSH;
#endif
...
#ifdef TRACE_LVL_3
ocrPrintf("////// Form Fx, Fy (k=%d) and A\n",k);RAG_FLUSH;
#endif
...
#ifdef TRACE_LVL_2
ocrPrintf("//// Allocate memory for output image (%dx%d)\n",image_params->Iy,image_params->Ix);RAG_FLUSH;
#endif
	ocrGuid_t output_dbg;
#ifdef RAG_DRAM
...
#endif
	if(output == NULL) {
		ocrPrintf("Error allocating memory for output.\n");RAG_FLUSH;
		xe_exit(1);
	}
