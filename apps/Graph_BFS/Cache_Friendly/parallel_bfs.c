...
int push(int **current_ref, int thread_id)
{
	...
	*current_ref = (int *) malloc(sizeof(int)*sum);
...
void BuildGraph(char *filename)
{
	...
	glbl_Adjacency = (int **) malloc(numNodes*sizeof(int *));
...
void parseEdgeList(char *str, int key, int BUFF_SIZE)
{
	...
	glbl_Adjacency[key] = (int *) malloc((neighbor_count+1)*sizeof(int));
...
void init_globals(int numThreads)
{
	...
	Communication_count = (int **) malloc(sizeof(int *)*numThreads);
	int i = 0;
	for(i = 0; i < numThreads; i++)
	{
		Communication[i] = (new_node **) malloc(sizeof(new_node *)*numThreads);
		Communication_count[i] = (int *) malloc(sizeof(int)*numThreads);
	...
	glbl_VIS = (char *) malloc(size);
	for(i = 0; i < size / sizeof(char); i++) glbl_VIS[i] = 0;

	glbl_DepthParent = (int *) malloc(2*sizeof(int)*numNodes); // one for depth, one for parent
