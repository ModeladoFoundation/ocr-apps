...
#define THRESHOLD_COMP 30//0//180
#include "common.h"

ocrGuid_t templateComputeSpawner;
ocrGuid_t event_guid[THRESHOLD_COMP];


int iteration;

ocrHint_t compute_app_type_hint;

ocrGuid_t wrap_task ( u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
  PRINTF("\nFinishing compute tasks"); fflush(stdout);
    if(instances==0)
      instances++;
    else
      ocrShutdown();
...
//Spawn more tasks before starting computation to generate enough parallelism
    ...
    if(index!=THRESHOLD_COMP-1){
        ocrEdtCreate/*Tag*/(&computeEdtGuid, templateComputeSpawner, 1, func_args_new, 1, NULL, PROPERTIES, affinity, NULL);//, tag);
        ocrSetHint(computeEdtGuid, &compute_app_type_hint);

        ocrGuid_t depv_guid;
        void* db;
        ocrDbCreate(&depv_guid, &db,
           sizeof(double), FLAGS, NULL_GUID, NO_ALLOC);
        ocrAddDependence(depv_guid, computeEdtGuid, 0, DB_DEFAULT_MODE);
...
ocrGuid_t mainEdt(u32 paramc, u64 *paramv, u32 depc, ocrEdtDep_t depv[]) {
  ...
  PRINTF("\n Compute main Edt");
    ...
    ocrEdtTemplateCreate(&templateWrap, wrap_task, 0, THRESHOLD_COMP);

    u64 app_tag = 0;

    ocrHintInit(&compute_app_type_hint, OCR_HINT_EDT_T);
    ocrSetHintValue(&compute_app_type_hint, OCR_HINT_EDT_TASK_TYPE, app_tag);
 
    ocrGuid_t wrapEdtGuid;
    u64 tag = 1;
    ocrEdtCreate/*Tag*/(&wrapEdtGuid, templateWrap, 0, NULL, THRESHOLD_COMP, NULL, PROPERTIES, affinity, NULL);//, tag);
    
    u64 i;
    for(i=0;i<THRESHOLD_COMP;i++){
        ocrEventCreate(&(event_guid[i]), OCR_EVENT_STICKY_T, TRUE);
        ocrAddDependence(event_guid[i], wrapEdtGuid, i, DB_MODE_ITW);
    }

    ocrEdtTemplateCreate(&templateComputeSpawner, spawn_and_compute_task, 1, 1);
   
    ocrGuid_t computeEdtGuid; 
    ocrEdtCreate/*Tag*/(&computeEdtGuid, templateComputeSpawner, 1, func_args, 1, NULL, PROPERTIES, affinity, NULL);//, tag);
