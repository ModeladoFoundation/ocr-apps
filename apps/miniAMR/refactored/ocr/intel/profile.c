...
//#include "block.h"
// TODO:  This goes away  #include "comm.h"
#include "proto.h"
#include "profile.h"
#include "control.h"
#include <string.h>
...
TODO?
   ...
   if (control->report_perf & 1) {
      fp = fopen("results.yaml", "w");
      fprintf(fp, "code: miniAMR\n");
      fprintf(fp, "version: %s\n", version);
      // TODO:  relevant?   fprintf(fp, "ranks: %d\n", num_pes);
      fprintf(fp, "npx: %d\n", control->npx);
      fprintf(fp, "npy: %d\n", control->npy);
      fprintf(fp, "npz: %d\n", control->npz);
      // TODO:  relevant?   fprintf(fp, "init_block_x: %d\n", init_block_x);
      // TODO:  relevant?   fprintf(fp, "init_block_y: %d\n", init_block_y);
      // TODO:  relevant?   fprintf(fp, "init_block_z: %d\n", init_block_z);
      fprintf(fp, "x_block_size: %d\n", control->x_block_size);
      fprintf(fp, "y_block_size: %d\n", control->y_block_size);
      fprintf(fp, "z_block_size: %d\n", control->z_block_size);
      // TODO:  relevant?   fprintf(fp, "reorder: %d\n", reorder);
      fprintf(fp, "permute: %d\n", control->permute);
      // TODO:  relevant?   fprintf(fp, "max_blocks_allowed: %d\n", max_num_blocks);
      fprintf(fp, "code: %d\n", control->code);
      fprintf(fp, "num_refine: %d\n", control->num_refine);
      fprintf(fp, "block_change: %d\n", control->block_change);
      fprintf(fp, "refine_ghost: %d\n", control->refine_ghost);
      fprintf(fp, "uniform_refine: %d\n", control->uniform_refine);
      fprintf(fp, "num_objects: %d\n", control->num_objects);

#if 0
Root progenitor has NOT been tracking movement of object(s).  If this is really needed for output, need to either compute full object movement (including bounce, etc.) or get one of the children to pass it up.
      ...
      // TODO:  relevant?   fprintf(fp, "nonblocking: %d\n", nonblocking);
      fprintf(fp, "checksum_freq: %d\n", control->checksum_freq);
      fprintf(fp, "refine_freq: %d\n", control->refine_freq);
      // TODO:  relevant?   fprintf(fp, "lb_opt: %d\n", lb_opt);
      // TODO:  relevant?   fprintf(fp, "inbalance: %d\n", inbalance);
      ...
      fprintf(fp, "   total_blocks_ts_min: %lf\n", nb_min);
      fprintf(fp, "   total_blocks_ts_max: %lf\n", nb_max);
...
void profile_report_perf_2and4 (FILE * fp, Control_t * control, Profile_t * profile, char * version) {
      ...
      fprintf(fp, "Run a %d x %d x %d grid\n", control->npx, control->npy, control->npz);    // fprintf(fp, "Run on %d ranks arranged in a %d x %d x %d grid\n", num_pes, npx, npy, npz);
      // TODO:  relevant?   fprintf(fp, "initial blocks per rank %d x %d x %d\n", init_block_x, init_block_y, init_block_z);
      fprintf(fp, "block size %d x %d x %d\n", control->x_block_size, control->y_block_size, control->z_block_size);
#if 0
      if (reorder)
...
#endif
      if (control->permute) fprintf(fp, "Order of exchanges permuted\n");
      // TODO:  relevant?   fprintf(fp, "Maximum number of blocks per rank is %d\n", max_num_blocks);
      // TODO:  relevant?   if (target_active) fprintf(fp, "Target number of blocks per rank is %d\n", target_active);
      // TODO:  relevant?   if (target_max) fprintf(fp, "Target max number of blocks per rank is %d\n", target_max);
      // TODO:  relevant?   if (target_min) fprintf(fp, "Target min number of blocks per rank is %d\n", target_min);
      if (control->code) fprintf(fp, "Code set to code %d\n", control->code);
      fprintf(fp, "Number of levels of refinement is %d\n", control->num_refine);
      fprintf(fp, "Blocks can change by %d levels per refinement step\n", control->block_change);
      if (control->refine_ghost) fprintf(fp, "Ghost cells will be used determine is block is refined\n");
      if (control->uniform_refine) {
...
// TODO:  relevant?
      ...
      fprintf(fp, "Calculate on %d variables with %d point stencil\n", control->num_vars, control->stencil);
      // TODO:  relevant?   fprintf(fp, "Communicate %d variables at a time\n", comm_vars);
...
void calculate_results(Profile_t * profile) {
...
#endif
   results[42] = profile->timer_refine_all;
#if 0
   ...
   gasket__mpi_Allreduce(glbl, results, average, 128, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD, -9999, __FILE__, __LINE__);
   gasket__mpi_Allreduce(glbl, results, minimum, 128, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD, -9999, __FILE__, __LINE__);
   gasket__mpi_Allreduce(glbl, results, maximum, 128, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD, -9999, __FILE__, __LINE__);

   for (i = 0; i < 128; i++) {
      average[i] /= (double) num_pes;
      stddev[i] = (results[i] - average[i])*(results[i] - average[i]);
   }
   gasket__mpi_Allreduce(glbl, stddev, stddev_sum, 128, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD, -9999, __FILE__, __LINE__);
   for (i = 0; i < 128; i++) {
...
void init_profile(BlockMeta_t * const meta)
{
   ...
   Profile_t * profile = meta->profileDb.base;
