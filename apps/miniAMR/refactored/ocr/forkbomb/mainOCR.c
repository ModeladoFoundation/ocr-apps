...
ocrGuid_t blockEdt( u32 paramc, u64 * paramv, u32 depc, ocrEdtDep_t depv[] )
{
    ...
    if( PRM_block.timestep < 1500 )
    {
        ...
        if( PRM_block.timestep % 50 == 0 )
        {
            ...
            ocrEdtTemplateCreate( &stencilTML, stencilEdt, pCount, 1 );
            ocrEdtCreate( &stencilGUID, stencilTML, EDT_PARAM_DEF, (u64 *)&PRM_block, EDT_PARAM_DEF, NULL,
            ...
            ocrEdtTemplateDestroy( stencilTML );
