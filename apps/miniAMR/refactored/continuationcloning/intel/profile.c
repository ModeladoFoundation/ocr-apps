...
#define i                 (lcl->i)
#define total_gflops      (lcl->total_gflops)
#define gflops_rank       (lcl->gflops_rank)
#define total_fp_ops      (lcl->total_fp_ops)
#define total_fp_adds     (lcl->total_fp_adds)
#define total_fp_divs     (lcl->total_fp_divs)
#define version           (lcl->version)
#define op                (lcl->pointers.op)
#define fp                (lcl->pointers.fp)

   SUSPENDABLE_FUNCTION_PROLOGUE(Frame__profile_t)
   TRACE
   fp = NULL;
   op = NULL;

   strcpy (version, "1.0 provisional");

   CALL_SUSPENDABLE_CALLEE(1)
   calculate_results(glbl);
...
#if 0
   // FIXME:  This exchange is here only because there is presently a termination bug that causes OCR to crash in one of the non-zero PEs before PE0 has a chance to report out.

printf ("pe = %d, Calling final sync operation\n", my_pe);
   just_junk = 1;
   CALL_SUSPENDABLE_CALLEE(1)
   gasket__mpi_Allreduce(glbl, &just_junk, average, 1, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, -9999, __FILE__, __LINE__);
   DEBRIEF_SUSPENDABLE_FUNCTION(;)
printf ("pe = %d, just_junk = %d\n", my_pe, just_junk);
#endif
...
void calculate_results(Globals_t * const glbl)
{
   typedef struct {
      ...
      Frame_Header_t calleeFrame;
   } Frame__calculate_results_t;

#define results           (lcl->results)
#define stddev_sum        (lcl->stddev_sum)
#define i                 (lcl->i)

   SUSPENDABLE_FUNCTION_PROLOGUE(Frame__calculate_results_t)
   TRACE
