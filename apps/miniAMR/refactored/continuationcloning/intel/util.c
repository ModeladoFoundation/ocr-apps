...
//#define PRINT_GASKET_DEBUG_MESSAGES
...
#define status (lcl->status)
   ...
   SUSPENDABLE_FUNCTION_PROLOGUE(Frame__gasket__mpi_Wait__for_Recv_t)
...
#define source (lcl->source)
   ...
   SUSPENDABLE_FUNCTION_PROLOGUE(Frame__gasket__mpi_Wait__for_Irecv_t)
#ifdef PRINT_GASKET_DEBUG_MESSAGES
if (line <= 1000000) {
    printf ("pe = %3d, %30s called from %20s line %7d; source is %d\n", my_pe, __func__, file, line, source_arg); fflush(stdout);
} else {
...
#define req    (lcl->req)
   ...
   SUSPENDABLE_FUNCTION_PROLOGUE(Frame__gasket__mpi_Send_t)
#ifdef PRINT_GASKET_DEBUG_MESSAGES
if (line <= 1000000) {
    printf ("pe = %3d, %30s called from %20s line %7d; dest = %d\n", my_pe, __func__, file, line, dest); fflush(stdout);
} else {
...
#define dest   (lcl->dest)
   ...
   SUSPENDABLE_FUNCTION_PROLOGUE(Frame__gasket__mpi_Isend_t)
#ifdef PRINT_GASKET_DEBUG_MESSAGES
if (line <= 1000000) {
    printf ("pe = %3d, %30s called from %20s line %7d; dest = %d\n", my_pe, __func__, file, line, dest_arg); fflush(stdout);
} else {
...
#define status      (lcl->status)
   ...
   SUSPENDABLE_FUNCTION_PROLOGUE(Frame__gasket__mpi_Bcast_t)
#ifdef PRINT_GASKET_DEBUG_MESSAGES
if (line <= 1000000) {
    printf ("pe = %3d, %30s called from %20s line %7d\n", my_pe, __func__, file, line); fflush(stdout);
} else {
...
#define comm        (lcl->comm)
   ...
   SUSPENDABLE_FUNCTION_PROLOGUE(Frame__gasket__mpi_Allreduce_t)
#ifdef PRINT_GASKET_DEBUG_MESSAGES
if (line <= 1000000) {
    printf ("pe = %3d, %30s called from %20s line %7d\n", my_pe, __func__, file, line); fflush(stdout);
} else {
...
#define status      (lcl->status)
   ...
   SUSPENDABLE_FUNCTION_PROLOGUE(Frame__gasket__mpi_Alltoall_t)
#ifdef PRINT_GASKET_DEBUG_MESSAGES
if (line <= 1000000) {
    printf ("pe = %3d, %30s called from %20s line %7d\n", my_pe, __func__, file, line); fflush(stdout);
} else {
...
void CapturePointerBaseAndOffset(Globals_t * const glbl, void * ptrToCapture, char * filename, const char * funcname, int linenum, int ptrnum) {
   long long ptrOffset;
   if ((((long long) topPtrAdjRec) + sizeof(PtrAdjustmentRecord_t)) >= ((long long) &glbl->stack[SIZEOFSTACK])) { \
      printf ("Stack overflow in Global_t block, detected in CapturePointerBaseAndOffset.  Increase SIZEOFSTACK and try again.\n");fflush(stdout); \
      exit(-1); \
   } \
   topPtrAdjRec++;

   if ((long long) ptrToCapture  == 0) {                                                // Deal with the nice case where this pointer is NULL.
      topPtrAdjRec->serialNum = 0;                                                      // Serial Number of NULL pointer.
      topPtrAdjRec->ptrOffset = 0xDEADBEEF;
      return;
   }

   ptrOffset = ((long long) ptrToCapture) - ((long long) glbl);
   if (ptrOffset >= 0 && ptrOffset < sizeof(Globals_t)) {                               // If within glbl datablock.
      topPtrAdjRec->serialNum = 1;                                                      // Serial Number of glbl datablock.
      topPtrAdjRec->ptrOffset = (int) ptrOffset;                                        // Offset relative to the start of the glbl datablock.
      return;
   }

   ptrOffset = ((long long) ptrToCapture) - ((long long) bulk);
   if (bulk != NULL && ptrOffset >= 0 && ptrOffset < sizeof_Bulk_t) {                   // If within bulk datablock.
      topPtrAdjRec->serialNum = 2;                                                      // Serial Number of bulk datablock.
      topPtrAdjRec->ptrOffset = (int) ptrOffset;                                        // Offset relative to the start of the bulk datablock.
      return;
   }

   ptrOffset = ((long long) ptrToCapture) - ((long long) send_buff);
   if (send_buff != NULL && ptrOffset >= 0 && ptrOffset < s_buf_size*sizeof(double)) {  // If within send_buff_t datablock.
      topPtrAdjRec->serialNum = 3;                                                      // Serial Number of send_buff_t datablock.
      topPtrAdjRec->ptrOffset = (int) ptrOffset;                                        // Offset relative to the start of the send_buff_t datablock.
      return;
   }

   ptrOffset = ((long long) ptrToCapture) - ((long long) recv_buff);
   if (recv_buff != NULL && ptrOffset >= 0 && ptrOffset < r_buf_size*sizeof(double)) {  // If within recv_buff_t datablock.
      topPtrAdjRec->serialNum = 4;                                                      // Serial Number of recv_buff_t datablock.
      topPtrAdjRec->ptrOffset = (int) ptrOffset;                                        // Offset relative to the start of the recv_buff_t datablock.
      return;
   }

   printf ("ERROR!!!!!  Enhance CapturePointerBaseAndOffset to deal with something that points to a different datablock than the one(s) dealt with so far.\n");
   printf ("            Note that there's a good chance this an occurance of an undefined pointer variable.  Initialize it to NULL to fix the problem.\n");
   printf ("PE=%d, file %s, function %s, line %d, ptr number %d\n", my_pe, filename, funcname, linenum, ptrnum);fflush(stdout);
   printf ("Pointer address is %p\n", ptrToCapture);
   printf ("glbl      datablock:        0x%016lx to 0x%016lx\n", (long long) glbl,      ((long long) glbl) + sizeof(Globals_t));
   printf ("bulk      datablock:        0x%016lx to 0x%016lx\n", (long long) bulk,      ((long long) bulk) + sizeof_Bulk_t);
   printf ("send_buff datablock:        0x%016lx to 0x%016lx\n", (long long) send_buff, ((long long) send_buff) + s_buf_size * sizeof(double));
   printf ("recv_buff datablock:        0x%016lx to 0x%016lx\n", (long long) recv_buff, ((long long) recv_buff) + r_buf_size * sizeof(double));
   exit(-1);
}
...
void RestorePointerBaseAndOffset(Globals_t * const glbl, void ** ptrToRestore, char * filename, const char * funcname, int linenum, int ptrnum) {
   ...
   switch (topPtrAdjRec->serialNum) {
   case 0:   // NULL pointer:
   {
      *ptrToRestore = NULL;
      break;
   }
   case 1:   // Pointer to something in the glbl datablock.
   {
      *ptrToRestore = ((void *) (((unsigned long long) glbl) + topPtrAdjRec->ptrOffset));
      break;
   }
   case 2:   // Pointer to something in the bulk datablock.
   {
      *ptrToRestore = ((void *) (((unsigned long long) bulk) + topPtrAdjRec->ptrOffset));
      break;
   }
   case 3:   // Pointer to something in the send_buff datablock.
   {
      *ptrToRestore = ((void *) (((unsigned long long) send_buff) + topPtrAdjRec->ptrOffset));
      break;
   }
   case 4:   // Pointer to something in the recv_buff datablock.
   {
      *ptrToRestore = ((void *) (((unsigned long long) recv_buff) + topPtrAdjRec->ptrOffset));
      break;
   }
   default:
   {
       printf ("PE = %d, Corrupt serialNum 0x%x taken from 0x%p for pointer in need of adjustment\n", my_pe, topPtrAdjRec->serialNum, topPtrAdjRec);fflush(stdout);
       *((int *) 0) = 123;
       exit(-1);
   }
   }
