...
void compute_hourglass_partial(cncTag_t iteration, cncTag_t element_id, volumeItem element_volume_in, sound_speedItem sound_speed_in, neighbor_positionItem *neighbor_position, neighbor_velocityItem *neighbor_velocity, luleshCtx *ctx) {
	...
	double element_volume = element_volume_in.item;
	...
	double coefficient = - ctx->constants.hgcoef * 0.01 * sound_speed *
			ctx->domain.element_mass[element_id] / cbrt(determ_value);
	...
	for(local_node_id = 0; local_node_id < 8; local_node_id++) {
		...
		for(local_element_id = 0; local_element_id < 8; local_element_id++) {
			if(ctx->mesh.nodes_element_neighbors[node_id][local_element_id] == element_id){
				...
				vector *hourglass_partial_out;
				cncHandle_t hourglass_partial_outHandle = cncCreateItem_hourglass_partial(&hourglass_partial_out);
				*hourglass_partial_out = forces_out[local_node_id];
				cncPut_hourglass_partial(hourglass_partial_outHandle, iteration, map_id, ctx);
			}
		}
	}
}
