 ...
 * typeof position_gradient is vector
 * typeof velocity_gradient is vector
 * typeof neighbor_velocity_gradient is vector
 */
void compute_viscosity_terms(cncTag_t iteration, cncTag_t element_id, volumeItem volume_in, volume_derivativeItem volume_derivative_in, position_gradientItem position_gradient_in, velocity_gradientItem velocity_gradient_in, neighbor_velocity_gradientItem *neighbor_velocity_gradient, luleshCtx *ctx) {
	...
	vector position_gradient = position_gradient_in.item;
	vector velocity_gradient = velocity_gradient_in.item;

	vector velocity_gradients[6];
	{ // Access "neighbor_velocity_gradient" inputs
		s64 _local_element_id;
		for (_local_element_id = 0; _local_element_id < ((6)-(0)); _local_element_id++) {
			velocity_gradients[_local_element_id] = neighbor_velocity_gradient[_local_element_id].item;
		}
	}
	...
	if (volume_derivative > 0.0) {
		...
		for(face_id = 0; face_id < 6; face_id++) {
			...
			if(ctx->mesh.elements_element_neighbors[element_id][face_id] >= 0) {
			...
			} else if(ctx->mesh.elements_element_neighbors[element_id][face_id] == -2) {
	...
	double *linear_viscosity_term_out;
	cncHandle_t linear_viscosity_term_outHandle = cncCreateItem_linear_viscosity_term(&linear_viscosity_term_out);
	*linear_viscosity_term_out = linear_viscosity_term;
	cncPut_linear_viscosity_term(linear_viscosity_term_outHandle, iteration, element_id, ctx);

	// Put "quadratic_viscosity_term_out" items
	double *quadratic_viscosity_term_out;
	cncHandle_t quadratic_viscosity_term_outHandle = cncCreateItem_quadratic_viscosity_term(&quadratic_viscosity_term_out);
	*quadratic_viscosity_term_out = quadratic_viscosity_term;
	cncPut_quadratic_viscosity_term(quadratic_viscosity_term_outHandle, iteration, element_id, ctx);
}
