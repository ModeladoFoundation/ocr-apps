 ...
 * typeof pressure is double
 * typeof viscosity is double
 * typeof linear_viscosity_term is double
 * typeof quadratic_viscosity_term is double
 */
void compute_energy(cncTag_t iteration, cncTag_t element_id, volumeItem volume_in, volumeItem previous_volume_in, energyItem previous_energy_in, pressureItem previous_pressure_in, viscosityItem previous_viscosity_in, linear_viscosity_termItem qlin_in, quadratic_viscosity_termItem qquad_in, luleshCtx *ctx) {
	...
	double previous_volume = previous_volume_in.item;
	double previous_energy = previous_energy_in.item;
	double previous_pressure = previous_pressure_in.item;
	double previous_viscosity = previous_viscosity_in.item;
	double qlin = qlin_in.item;
	double qquad = qquad_in.item;
	...
	double *energy_out;
	cncHandle_t energy_outHandle = cncCreateItem_energy(&energy_out);
	*energy_out = energy;
	cncPut_energy(energy_outHandle, iteration, element_id, ctx);

	// Put "pressure_out" items
	double *pressure_out;
	cncHandle_t pressure_outHandle = cncCreateItem_pressure(&pressure_out);
	*pressure_out = pressure;
	cncPut_pressure(pressure_outHandle, iteration, element_id, ctx);

	// Put "viscosity_out" items
	double *viscosity_out;
	cncHandle_t viscosity_outHandle = cncCreateItem_viscosity(&viscosity_out);
	*viscosity_out = viscosity;
	cncPut_viscosity(viscosity_outHandle, iteration, element_id, ctx);

	// Put "sound_speed_out" items
	double *sound_speed_out;
	cncHandle_t sound_speed_outHandle = cncCreateItem_sound_speed(&sound_speed_out);
	*sound_speed_out = sound_speed;
	cncPut_sound_speed(sound_speed_outHandle, iteration, element_id, ctx);
