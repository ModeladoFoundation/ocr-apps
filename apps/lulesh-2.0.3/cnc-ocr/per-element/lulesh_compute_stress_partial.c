...
void compute_stress_partial(cncTag_t iteration, cncTag_t element_id, pressureItem pressure_in, viscosityItem viscosity_in, neighbor_positionItem *neighbor_position, luleshCtx *ctx) {
	...
	// INPUTS
	//
	double pressure = pressure_in.item;
	double viscosity = viscosity_in.item;
	...
	for(local_node_id = 0; local_node_id < 8; local_node_id++) {
		...
		for(local_element_id = 0; local_element_id < 8; local_element_id++) {
			if(ctx->mesh.nodes_element_neighbors[node_id][local_element_id]==element_id){
				...
				vector *stress_partial_out;
				cncHandle_t stress_partial_outHandle = cncCreateItem_stress_partial(&stress_partial_out);
				*stress_partial_out = forces_out[local_node_id];
				cncPut_stress_partial(stress_partial_outHandle, iteration, map_id, ctx);
			}
		}
	}
