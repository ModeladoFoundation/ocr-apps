 ...
 * typeof neighbor_velocity is vector
 */
void compute_gradients(cncTag_t iteration, cncTag_t element_id, volumeItem volume_in, neighbor_positionItem *neighbor_position, neighbor_velocityItem *neighbor_velocity, luleshCtx *ctx) {
	...
	vector *position_gradient_out;
	cncHandle_t position_gradient_outHandle = cncCreateItem_position_gradient(&position_gradient_out);
	*position_gradient_out = position_gradient;
	cncPut_position_gradient(position_gradient_outHandle, iteration, element_id, ctx);

	// Put "velocity_gradient_out" items
	vector *velocity_gradient_out;
	cncHandle_t velocity_gradient_outHandle = cncCreateItem_velocity_gradient(&velocity_gradient_out);
	*velocity_gradient_out = velocity_gradient;
	cncPut_velocity_gradient(velocity_gradient_outHandle, iteration, element_id, ctx);
