...
void lulesh_init(luleshArgs *args, luleshCtx *ctx) {
	...
	*velocity_gradient = vector_new(0.0, 0.0, 0.0);
    ...
    { // Put "force" items
        ...
        for (_node_id = 0; _node_id < ctx->nodes; _node_id++) {
            vector *force = cncCreateItem_force();
    ...
    { // Put "position" items
        s64 _node_id;
        for (_node_id = 0; _node_id < ctx->nodes; _node_id++) {
            vertex *position = cncCreateItem_position();
    ...
    { // Put "velocity" items
        s64 _node_id;
        for (_node_id = 0; _node_id < ctx->nodes; _node_id++) {
            vector *velocity = cncCreateItem_velocity();
    ...
    { // Put "volume" items
        ...
        for (_element_id = 0; _element_id < ctx->elements; _element_id++) {
            double *volume = cncCreateItem_volume();
    ...
    { // Put "viscosity" items
        s64 _element_id;
        for (_element_id = 0; _element_id < ctx->elements; _element_id++) {
            double *viscosity = cncCreateItem_viscosity();
    ...
    { // Put "pressure" items
        s64 _element_id;
        for (_element_id = 0; _element_id < ctx->elements; _element_id++) {
            double *pressure = cncCreateItem_pressure();
    ...
    { // Put "energy" items
        s64 _element_id;
        for (_element_id = 0; _element_id < ctx->elements; _element_id++) {
            double *energy = cncCreateItem_energy();
    ...
    { // Put "sound_speed" items
        s64 _element_id;
        for (_element_id = 0; _element_id < ctx->elements; _element_id++) {
            double *sound_speed = cncCreateItem_sound_speed();
    ...
    double *elapsed_time = cncCreateItem_elapsed_time();
	*elapsed_time = ctx->domain.initial_delta_time; // Count the first time step
    ...
    double *delta_time = cncCreateItem_delta_time();
...
void lulesh_finalize(double final_origin_energy, luleshCtx *ctx) {
