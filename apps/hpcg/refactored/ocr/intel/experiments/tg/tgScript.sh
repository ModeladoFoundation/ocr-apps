...
server_nodes=(61 62 63)
client_nodes=(64 65 66 67) # 236 237 238 239 240 241 242)
...
for BLOCKCOUNT in ${BLOCKCOUNTS[@]}; do
    ...
    for iter in ${iters[@]}; do
        ...
        BUILD_CMD="MACHINE_CONFIG=$PWD/${machineConfigFile} WORKLOAD_INSTALL_ROOT=./${winstall} make -f Makefile.tg install WORKLOAD_ARGS='$((2*${rx})) $((2*${ry})) $((2*${rz})) ${size} ${iter} 0'"
        eval $BUILD_CMD
