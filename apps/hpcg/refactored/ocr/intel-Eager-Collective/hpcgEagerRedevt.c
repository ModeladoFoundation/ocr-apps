...
January 2017: modified to support EAGER data blocks
   (reuse datablocks, declare them EAGER)
...
typedef struct{
    ...
    ocrGuid_t redEvtGuid;
    ocrGuid_t redEvtTimerGuid;
...
//offsets in bytes for the various structures in the datablock beyond the private structure
    ...
    ocrGuid_t packTML;
    ocrGuid_t unpackTML;
    ocrGuid_t haloSendEVT[26][2];
    ocrGuid_t haloRecvEVT[26][2];
...
//make smooth wait for both Pack and Unpack
        ...
        ocrEdtCreate(&smoothEDT, pbPTR->smoothTML, EDT_PARAM_DEF, (u64 *) smoothPRM, EDT_PARAM_DEF, NULL, EDT_PROP_NONE, &pbPTR->myEdtAffinityHNT, NULL);
...
//launch halo
        ...
        ocrEdtCreate(&haloExchangeEDT, pbPTR->haloExchangeTML, EDT_PARAM_DEF, (u64 *) haloExchangePRM, EDT_PARAM_DEF, NULL, EDT_PROP_NONE, &pbPTR->myEdtAffinityHNT, NULL);
#ifdef TIMER
        pbPTR->end[pbPTR->timestep][timePhase] = getTime();
#endif
        ocrDbRelease(pbDBK);
...
//make spmv depend on pack and unpack
        ...
        ocrEdtCreate(&spmvEDT, pbPTR->spmvTML, EDT_PARAM_DEF, (u64 *) spmvPRM, EDT_PARAM_DEF, NULL, EDT_PROP_NONE, &pbPTR->myEdtAffinityHNT, NULL);
        ocrEventCreate(&unpackEVT, OCR_EVENT_ONCE_T, EVT_PROP_TAKES_ARG);
        ocrEventCreate(&packEVT, OCR_EVENT_ONCE_T, EVT_PROP_TAKES_ARG);
...
//needed because the template is used after the release of the shared block
        ...
        ocrEdtCreate(&mgEDT, sbMgTML, EDT_PARAM_DEF, (u64 *) mgPRM, EDT_PARAM_DEF, NULL, EDT_PROP_NONE, &pbPTR->myEdtAffinityHNT, NULL);
...
*/
    ...
    ocrGuid_t redEvtGuid = pbPTR->redEvtGuid;
    u32 myrank = pbPTR->myrank;
...
if(pbPTR->debug > 0)
    PRINTF("CG%d T%d P%d begin private "GUIDF" reduceprivate "GUIDF" mydata "GUIDF" \n", myrank, timestep, phase, GUIDA(pbDBK), GUIDA(rpDBK), GUIDA(myDataDBK));
...
#ifdef AFFINITY
        ocrEdtCreate(&hpcgEDT, pbPTR->hpcgTML, EDT_PARAM_DEF, NULL, EDT_PARAM_DEF, NULL, EDT_PROP_NONE, &pbPTR->myEdtAffinityHNT, NULL);
#else
...
#ifndef NO_REDUCTION
        ocrAddDependenceSlot(redEvtGuid, myrank, rpPTR->returnEVT, 0, DB_MODE_RO);
#endif
        //TODO: not sure if the channel is important here
    ...
    for(j=0;j<PHASES;j++)
        ...
        */
            ...
            for(i=0;i<pbPTR->mt[0];i++) sum += (1-x[i])*(1-x[i]);
                //TODO set up a new event for the reduce
            *myDataPTR = sum;
        // TODO: who depends on that ? everybody or just rank zero ?
#ifdef RANK_TIMER
...
#endif
            if (myrank == 0) {
                ocrAddDependenceSlot(pbPTR->redEvtTimerGuid, myrank, pbPTR->finalOnceEVT, 0, DB_MODE_RO);
            } else {
                ocrGuid_t dummyEVT;
...
//record received channel events
    ...
    for(i=0;i<26;i++) {
        if(!ocrGuidIsNull(DEPVARRAY(channelInit,channel,i,guid))) {
            pbPTR->haloSendEVT[i][0] = ((ocrGuid_t *)DEPVARRAY(channelInit,channel,i,ptr))[0];
            pbPTR->haloSendEVT[i][1] = ((ocrGuid_t *)DEPVARRAY(channelInit,channel,i,ptr))[1];
            ocrDbDestroy(DEPVARRAY(channelInit,channel,i,guid));
          } else {
            pbPTR->haloRecvEVT[i][0] = NULL_GUID;
            pbPTR->haloRecvEVT[i][1] = NULL_GUID;
          }
    }

if(pbPTR->debug > 0) for(i=0;i<26;i++) PRINTF("CI%d ind %d recv "GUIDF" "GUIDF" send "GUIDF" "GUIDF" \n", pbPTR->myrank, i,
pbPTR->haloRecvEVT[i][0],
pbPTR->haloRecvEVT[i][1],
pbPTR->haloSendEVT[i][0],
pbPTR->haloSendEVT[i][1]);

//launch hpcg

    ocrGuid_t hpcgEDT;
    ocrEdtCreate(&hpcgEDT, pbPTR->hpcgTML, EDT_PARAM_DEF, NULL, EDT_PARAM_DEF, NULL,
      EDT_PROP_NONE, &pbPTR->myEdtAffinityHNT, NULL);
...
//reduction block
    //RE: redevt creation
    //RE: Need to have add the reduction event GUID passed in here to invoke create
    ...
    paramsChan.EVENT_CHANNEL.maxGen = 2;
    paramsChan.EVENT_CHANNEL.nbSat = 1;
    paramsChan.EVENT_CHANNEL.nbDeps = 1;

    ocrEventCreateParams(&(rpPTR->returnEVT), OCR_EVENT_CHANNEL_T, false, &paramsChan);
    ...
    pbPTR->redEvtTimerGuid = sbPTR->redEvtTimerGuid;
    pbPTR->redEvtGuid = sbPTR->redEvtGuid;
...
//initialize sendBlock, sendEVT, and recvEVT
    ...
    for(i=0;i<26;i++) {
        pbPTR->haloSendEVT[i][0] = UNINITIALIZED_GUID;
        pbPTR->haloSendEVT[i][1] = UNINITIALIZED_GUID;
    }


    if(pz == 0) {
        pbPTR->haloSendEVT[0][0] = NULL_GUID;
        pbPTR->haloSendEVT[1][0] = NULL_GUID;
        pbPTR->haloSendEVT[2][0] = NULL_GUID;
        pbPTR->haloSendEVT[3][0] = NULL_GUID;
        pbPTR->haloSendEVT[4][0] = NULL_GUID;
        pbPTR->haloSendEVT[5][0] = NULL_GUID;
        pbPTR->haloSendEVT[6][0] = NULL_GUID;
        pbPTR->haloSendEVT[7][0] = NULL_GUID;
        pbPTR->haloSendEVT[8][0] = NULL_GUID;
        pbPTR->haloSendEVT[0][1] = NULL_GUID;
        pbPTR->haloSendEVT[1][1] = NULL_GUID;
        pbPTR->haloSendEVT[2][1] = NULL_GUID;
        pbPTR->haloSendEVT[3][1] = NULL_GUID;
        pbPTR->haloSendEVT[4][1] = NULL_GUID;
        pbPTR->haloSendEVT[5][1] = NULL_GUID;
        pbPTR->haloSendEVT[6][1] = NULL_GUID;
        pbPTR->haloSendEVT[7][1] = NULL_GUID;
        pbPTR->haloSendEVT[8][1] = NULL_GUID;
    }

    if(pz == pbPTR->npz-1) {
        pbPTR->haloSendEVT[17][0] = NULL_GUID;
        pbPTR->haloSendEVT[18][0] = NULL_GUID;
        pbPTR->haloSendEVT[19][0] = NULL_GUID;
        pbPTR->haloSendEVT[20][0] = NULL_GUID;
        pbPTR->haloSendEVT[21][0] = NULL_GUID;
        pbPTR->haloSendEVT[22][0] = NULL_GUID;
        pbPTR->haloSendEVT[23][0] = NULL_GUID;
        pbPTR->haloSendEVT[24][0] = NULL_GUID;
        pbPTR->haloSendEVT[25][0] = NULL_GUID;
        pbPTR->haloSendEVT[17][1] = NULL_GUID;
        pbPTR->haloSendEVT[18][1] = NULL_GUID;
        pbPTR->haloSendEVT[19][1] = NULL_GUID;
        pbPTR->haloSendEVT[20][1] = NULL_GUID;
        pbPTR->haloSendEVT[21][1] = NULL_GUID;
        pbPTR->haloSendEVT[22][1] = NULL_GUID;
        pbPTR->haloSendEVT[23][1] = NULL_GUID;
        pbPTR->haloSendEVT[24][1] = NULL_GUID;
        pbPTR->haloSendEVT[25][1] = NULL_GUID;
    }
    ...
    if(py == 0) {
        ...
        pbPTR->haloSendEVT[1][0] = NULL_GUID;
        pbPTR->haloSendEVT[2][0] = NULL_GUID;
        pbPTR->haloSendEVT[9][0] = NULL_GUID;
        pbPTR->haloSendEVT[10][0] = NULL_GUID;
        pbPTR->haloSendEVT[11][0] = NULL_GUID;
        pbPTR->haloSendEVT[17][0] = NULL_GUID;
        pbPTR->haloSendEVT[18][0] = NULL_GUID;
        pbPTR->haloSendEVT[19][0] = NULL_GUID;
        pbPTR->haloSendEVT[0][1] = NULL_GUID;
        pbPTR->haloSendEVT[1][1] = NULL_GUID;
        pbPTR->haloSendEVT[2][1] = NULL_GUID;
        pbPTR->haloSendEVT[9][1] = NULL_GUID;
        pbPTR->haloSendEVT[10][1] = NULL_GUID;
        pbPTR->haloSendEVT[11][1] = NULL_GUID;
        pbPTR->haloSendEVT[17][1] = NULL_GUID;
        pbPTR->haloSendEVT[18][1] = NULL_GUID;
        pbPTR->haloSendEVT[19][1] = NULL_GUID;
    }
    ...
    if(py == pbPTR->npy-1) {
        ...
        pbPTR->haloSendEVT[7][0] = NULL_GUID;
        pbPTR->haloSendEVT[8][0] = NULL_GUID;
        pbPTR->haloSendEVT[14][0] = NULL_GUID;
        pbPTR->haloSendEVT[15][0] = NULL_GUID;
        pbPTR->haloSendEVT[16][0] = NULL_GUID;
        pbPTR->haloSendEVT[23][0] = NULL_GUID;
        pbPTR->haloSendEVT[24][0] = NULL_GUID;
        pbPTR->haloSendEVT[25][0] = NULL_GUID;
        pbPTR->haloSendEVT[6][1] = NULL_GUID;
        pbPTR->haloSendEVT[7][1] = NULL_GUID;
        pbPTR->haloSendEVT[8][1] = NULL_GUID;
        pbPTR->haloSendEVT[14][1] = NULL_GUID;
        pbPTR->haloSendEVT[15][1] = NULL_GUID;
        pbPTR->haloSendEVT[16][1] = NULL_GUID;
        pbPTR->haloSendEVT[23][1] = NULL_GUID;
        pbPTR->haloSendEVT[24][1] = NULL_GUID;
        pbPTR->haloSendEVT[25][1] = NULL_GUID;
    }
    ...
    if(px == 0) {
        ...
        pbPTR->haloSendEVT[6][0] = NULL_GUID;
        pbPTR->haloSendEVT[9][0] = NULL_GUID;
        pbPTR->haloSendEVT[12][0] = NULL_GUID;
        pbPTR->haloSendEVT[14][0] = NULL_GUID;
        pbPTR->haloSendEVT[17][0] = NULL_GUID;
        pbPTR->haloSendEVT[20][0] = NULL_GUID;
        pbPTR->haloSendEVT[23][0] = NULL_GUID;
        ...
        pbPTR->haloSendEVT[6][1] = NULL_GUID;
        pbPTR->haloSendEVT[9][1] = NULL_GUID;
        pbPTR->haloSendEVT[12][1] = NULL_GUID;
        pbPTR->haloSendEVT[14][1] = NULL_GUID;
        pbPTR->haloSendEVT[17][1] = NULL_GUID;
        pbPTR->haloSendEVT[20][1] = NULL_GUID;
        pbPTR->haloSendEVT[23][1] = NULL_GUID;
    ...
    if(px == pbPTR->npx-1) {
        ...
        pbPTR->haloSendEVT[8][0] = NULL_GUID;
        pbPTR->haloSendEVT[11][0] = NULL_GUID;
        pbPTR->haloSendEVT[13][0] = NULL_GUID;
        pbPTR->haloSendEVT[16][0] = NULL_GUID;
        pbPTR->haloSendEVT[19][0] = NULL_GUID;
        pbPTR->haloSendEVT[22][0] = NULL_GUID;
        pbPTR->haloSendEVT[25][0] = NULL_GUID;
        ...
        pbPTR->haloSendEVT[8][1] = NULL_GUID;
        pbPTR->haloSendEVT[11][1] = NULL_GUID;
        pbPTR->haloSendEVT[13][1] = NULL_GUID;
        pbPTR->haloSendEVT[16][1] = NULL_GUID;
        pbPTR->haloSendEVT[19][1] = NULL_GUID;
        pbPTR->haloSendEVT[22][1] = NULL_GUID;
        pbPTR->haloSendEVT[25][1] = NULL_GUID;
    ...
    ocrHintInit(&pbPTR->myEdtAffinityHNT,OCR_HINT_EDT_T);

#ifdef AFFINITY
    ocrGuid_t myEdtAffinity;
    ocrAffinityGetCurrent(&myEdtAffinity);
    ocrSetHintValue(&pbPTR->myEdtAffinityHNT, OCR_HINT_EDT_AFFINITY, ocrAffinityToHintValue(myEdtAffinity));
    ocrEdtCreate(&channelInitEDT, channelInitTML, EDT_PARAM_DEF, NULL, EDT_PARAM_DEF, NULL, EDT_PROP_NONE, &pbPTR->myEdtAffinityHNT, NULL);
#else
...
if(pbPTR->debug > 1)
    PRINTF("ind %d \n", ind);
                if(ocrGuidIsNull(pbPTR->haloSendEVT[ind][0])) {
                    pbPTR->haloRecvEVT[ind][0] = NULL_GUID;
                    pbPTR->haloRecvEVT[ind][1] = NULL_GUID;
...
//send
                    ...
                    ocrEventCreateParams(&(pbPTR->haloRecvEVT[ind][0]), OCR_EVENT_CHANNEL_T, false, &paramsChan);
                    ocrEventCreateParams(&(pbPTR->haloRecvEVT[ind][1]), OCR_EVENT_CHANNEL_T, false, &paramsChan);
...
//launch
    ...
    ocrDbRelease(pbDBK);
    ocrAddDependence(pbDBK, channelInitEDT, 0, DB_MODE_RW);
    ocrDbRelease(rpDBK);
    ocrAddDependence(rpDBK, channelInitEDT, 1, DB_MODE_RW);
...
*/
    ...
    for(myrank=0;myrank<nrank;myrank++) {
        ...
        // PRINTF("rank=%"PRIu64" mapped to PD %"PRIu64"\n", myrank, myPD);
        ocrAffinityGetAt(AFFINITY_PD, myPD, &(myEdtAffinity));
        ocrHint_t myHNT;
        ocrHintInit(&myHNT,OCR_HINT_EDT_T);
        ocrSetHintValue(&myHNT, OCR_HINT_EDT_AFFINITY, ocrAffinityToHintValue(myEdtAffinity));
