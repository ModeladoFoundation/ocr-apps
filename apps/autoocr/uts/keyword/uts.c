...
int uts_paramsToStr(char *strBuf, int ind) {
  ...
  if (type == BALANCED) {
    ...
    ind += sprintf(strBuf+ind, "        Expected size: %lu nodes, %lu leaves\n",
...
void uts_printParams() {
  ...
  if (verbose > 0) {
    ...
    ind += sprintf(strBuf+ind, "Execution strategy:  Parallel Recursive Search\n");
    printf("%s\n",strBuf);
  }
}
...
void __task uts_TreeSearch(Node * parent) {
  ...
  if (numChildren > 0) {
    ...
    __data Node *child = malloc( numChildren * sizeof(Node) );

    while ( childHeight > maxDepth ) (void) __sync_val_compare_and_swap (&maxDepth, maxDepth, childHeight);
    __sync_fetch_and_add(&nNodes,numChildren);

    for (int i = 0; i < numChildren; i++) {

        child[i].type   = childType;
        child[i].height = childHeight;
...
//        cilk_spawn uts_TreeSearch(&child[i]);
        uts_TreeSearch(&child[i]);
...
//    cilk_sync;
    free(child);
      ...
      __sync_fetch_and_add(&nLeaves,1);
...
int main(int argc, char *argv[]) {
  ...
  printf("UTS Tree size = %ld, ",nNodes);
  printf("tree depth  = %ld, ",maxDepth);
  printf("num leaves = %ld, ",nLeaves);
  printf(" (%5.2f%c) time = %f s\n",((double)nLeaves/(double)nNodes)*100.0,'%',et);
