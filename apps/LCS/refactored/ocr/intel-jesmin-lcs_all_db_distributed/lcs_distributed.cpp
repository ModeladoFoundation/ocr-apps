...
ocrGuid_t shutDownEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    #ifdef CHECK_RESULTS
    if(score[len*len-1]!=(int)paramv[1]) printf("Result did not match! :(\n");
    else printf("Result matched! We are good :)\n");
    #endif
...
extern "C" ocrGuid_t mainEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[] )
{
   ...
   // Create mapguids for S, T and lcs_score matrix.
    ...
    for(j = 1; j <grid_block_size; j++)
    {
        ...
        int * lcs_score_t = lcs_score[t];
        int start_val=(j-1)*base+1;
        //initialize
        for(int k=0; k<base;k++)
        {
                lcs_score_t[k]=start_val+k;
    ...
    ocrAddDependence( NULL_GUID, recLCSEdt, 0, DB_MODE_NULL );
