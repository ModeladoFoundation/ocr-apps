...
pthread_t _M4_threadsTable[MAX_SPLASH2_THREADS];
pthread_mutexattr_t _M4_normalMutexAttr;
int _M4_numSplashThreads = MAX_SPLASH2_THREADS;
...
static int stealTasks( long myThreadId, long myQ) 
{
#define COMPUTE_SOCKET(mQ, numTasks) ((nsockets == 1) ? 0 : (mQ >= (numTasks>>1)))
    int i, stolen = 0;
     __declspec(align(64)) int Neighbor[32];
     int threads_per_core = (nsockets == 1) ? numTaskQs : (numTaskQs>>1);
     int mySocket = COMPUTE_SOCKET(myQ, numTaskQs);
     int offset = 0;
     if ( (nsockets == 2) && (myThreadId >= (numTaskQs>>1))) offset = (numTaskQs>>1);

     int myQ_local = (mySocket == 0) ? myQ : (myQ - offset);
     int k;


     for(k=0; k<threads_per_core; k+= 2)
     {
         int delta0 = 1 + (k>>1);
         int delta1 = threads_per_core -1 - (k>>1);

         Neighbor[k+0] = offset + (((myQ_local + delta0) >= threads_per_core) ? (myQ_local + delta0 - threads_per_core) : (myQ_local + delta0));
         Neighbor[k+1] = offset + (((myQ_local + delta1) >= threads_per_core) ? (myQ_local + delta1 - threads_per_core) : (myQ_local + delta1));
     }

#if 0
     if (myQ == 0)
     {
         for(k=0; k<(threads_per_core-1); k++) printf("%d ", Neighbor[k]); printf("\n");
     }
#endif

    TRACE;

    for(k=0; k<(threads_per_core-1); k++)
    {
        i = Neighbor[k];
        if (COMPUTE_SOCKET(i,numTaskQs) != mySocket)  exit(123);
...
static void *taskQIdleLoop( void *arg) {
    ...
    // Anthony: add affinity for non-main thread here
...
void taskQInit( int numOfThreads, int maxNumOfTasks) {
    ...
    _M4_numSplashThreads = numOfThreads;
    {
        int _M4_i;
        for ( _M4_i = 0; _M4_i < MAX_SPLASH2_THREADS; _M4_i++) {
...
;
    ...
    printf( "\t\t <<<<<<<<<<<< This version has been built for Graph work and plays with task scheduling and stealing protocol >>>>>>>>>>>\n");
...
;
    ...
    // Anthony: Add affinity for main thread
    Set_Affinity(0);
