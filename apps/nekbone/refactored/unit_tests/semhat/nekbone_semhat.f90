!-----------------------------------------------------------------------
! The code for the procedure "semhat" was taken from
...
!-----------------------------------------------------------------------
      ...
      ! If any of these values changes, make also the change in nekbone_semhat.c
      ...
      do pDOF=firstdegree,lastdegree
        ...
        call semhat(ah,bh,ch,dh,zpts,wght,pDOF-1)

        write(*,'(A, 1I10)') 'pDOF= ', pDOF
        do k=1,pDOF
            write(*,'(1I10,2ES23.14)') k-1, bh(k), zpts(k)
        enddo
        do k=1,(2*N+1)
            write(*,'(A,1I10,1ES23.14)') 'W(k)=', k-1, wght(k)
        enddo

        call printout_matrices(pDOF-1, ah, ch, dh)
...
!--------------------------------------------------------------------
      SUBROUTINE printout_matrices(n, a,c,d)
        integer n
        real a(0:n,0:n),c(0:n,0:n),d(0:n,0:n)

        integer i,j

        do j=0,n
            do i=0,n
                write(*,'(2I10,3ES23.14)') i,j, a(i,j), c(i,j), d(i,j)
            end do
        end do
