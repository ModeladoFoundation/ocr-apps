...
static inline int checkEnv1(const char *k) {
    char * v = getenv(k);
    return !v || (v[0] == '1');
}
...
ocrGuid_t shutdownEdt ( u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    double secondsRun = endTime.tv_sec - _startTime.tv_sec;
    secondsRun += (endTime.tv_usec - _startTime.tv_usec) / 1000000.0;
...
ocrGuid_t smith_waterman_task ( u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    if ( i == n_tiles_height && j == n_tiles_width ) {
        ...
        ocrGuid_t end_event_guid = smithWatermanParamvIn->end_event_guid;
        ocrEventSatisfy(end_event_guid, NULL_GUID);
    }
    return NULL_GUID;
}
...
ocrGuid_t mainEdt ( u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    ocrEventCreate(&smithWatermanParamv.end_event_guid, OCR_EVENT_STICKY_T, EVT_PROP_NONE);
    ...
    ocrEventCreate(&dbParamsGuid, OCR_EVENT_STICKY_T, EVT_PROP_TAKES_ARG);
    ocrGuid_t dbParamsGuid2;
    u64 *params;
    ocrDbCreate(&dbParamsGuid2, (void **)&params, dbSize, DB_PROP_NONE, NULL_HINT, NO_ALLOC);
    ...
    ocrDbRelease(dbParamsGuid2);

    u64 affinityCount;
    ocrAffinityCount(AFFINITY_PD, &affinityCount);

    ocrGuid_t affinitiesGuid;
    ocrGuid_t *affinities;
    ocrDbCreate(&affinitiesGuid, (void **)&affinities,
            sizeof(*affinities)*affinityCount, DB_PROP_NONE, NULL_HINT, NO_ALLOC);
    ocrAffinityGet(AFFINITY_PD, &affinityCount, affinities);

    const int BAD_AFFINITIES = checkEnv1("BAD_AFFINITIES");
    ...
    for ( i = 1; i < n_tiles_height+1; ++i ) {
        for ( j = 1; j < n_tiles_width+1; ++j ) {
            // hint
            if (!BAD_AFFINITIES) {
                ocrSetHintValue(&hint, OCR_HINT_EDT_PRIORITY, i*10000 + j);
            } else {
                ocrSetHintValue(&hint, OCR_HINT_EDT_PRIORITY, j*10000 + i);
            }
            ocrSetHintValue(&hint, OCR_HINT_EDT_AFFINITY,
                    ocrAffinityToHintValue(affinities[(i/5) % affinityCount]));
    ...
    ocrGuid_t currentPlace;
    ocrAffinityGetCurrent(&currentPlace);
    ocrHintInit(&hint, OCR_HINT_EDT_T);
    ocrSetHintValue(&hint, OCR_HINT_EDT_AFFINITY, ocrAffinityToHintValue(currentPlace));

    ocrGuid_t shutdown_task_template_guid;
    ocrEdtTemplateCreate(&shutdown_task_template_guid, shutdownEdt, 0 /*paramc*/, 1 /*depc*/);
    ocrGuid_t shutdown_guid;
    ocrEdtCreate(&shutdown_guid, shutdown_task_template_guid,
            EDT_PARAM_DEF, NULL /*paramv*/,
            EDT_PARAM_DEF, &smithWatermanParamv.end_event_guid /*depv*/,
            EDT_PROP_NONE, &hint /*hint*/, NULL /*outputEvent*/);
    ...
    ocrEventSatisfy(dbParamsGuid, dbParamsGuid2);
    return NULL_GUID;
}
