...
inline static void sequential_cholesky_task_prescriber (ocrGuid_t edtTemp, int k,
                                                        int tileSize, ocrGuid_t*** lkji_event_guids, int chunk_size) {
    ...
    ocrGuid_t affinity = (ocrGuid_t)(k / chunk_size);
...
inline static void trisolve_task_prescriber ( ocrGuid_t edtTemp, int k, int j, int tileSize,
                                              ocrGuid_t*** lkji_event_guids, int chunk_size) {
    ...
    ocrGuid_t affinity = (ocrGuid_t)(j / chunk_size);
    ocrEdtCreate(&trisolve_task_guid, edtTemp, 4, func_args, 2, NULL, PROPERTIES, affinity, NULL);

    ocrAddDependence(lkji_event_guids[j][k][k], trisolve_task_guid, 0, DB_MODE_ITW);   // Update - 1: 66
    ocrAddDependence(lkji_event_guids[k][k][k+1], trisolve_task_guid, 1, DB_MODE_ITW); // Cholesky: 34
}
...
inline static void update_nondiagonal_task_prescriber ( ocrGuid_t edtTemp, int k, int j, int i,
    ...
    ocrAddDependence(lkji_event_guids[j][i][k], update_nondiagonal_task_guid, 0, DB_MODE_ITW);   // Update - 1: 66
    ocrAddDependence(lkji_event_guids[j][k][k+1], update_nondiagonal_task_guid, 1, DB_MODE_ITW); // Trisolve same row: 33
    ocrAddDependence(lkji_event_guids[i][k][k+1], update_nondiagonal_task_guid, 2, DB_MODE_ITW); // Trisolve other: 1
}
...
inline static void update_diagonal_task_prescriber ( ocrGuid_t edtTemp, int k, int j, int i,
    ...
    ocrAddDependence(lkji_event_guids[j][j][k], update_diagonal_task_guid, 0, DB_MODE_ITW);   // Update - 1: 66
    ocrAddDependence(lkji_event_guids[j][k][k+1], update_diagonal_task_guid, 1, DB_MODE_ITW); // Trisolve: 33
}
...
PAPI_start(eventset);
    ...
    int chunk_size = 1;//numTiles / NUM_WORKERS;
    ...
    ocrGuid_t hintSeq;
    ocrHintCreate(&hintSeq);
    ocrHintSetPriority(hintSeq, 0);
    ocrHintSetAccess0Slot(hintSeq, 0);
    ocrHintSetAccess0Weight(hintSeq, 100);
    ocrEdtTemplateCreateWithHints(&templateSeq, sequential_cholesky_task, 3, 1, hintSeq);

    ocrGuid_t hintTrisolve;
    ocrHintCreate(&hintUpdateTrisolve);
    //ocrHintSetEDTPriority(hintTrisolve, 1); // sets EDT priority
    //ocrHintSetDependencePriority(hintTrisolve, 0, dep_num, dep_wt); // slot with max data
    ocrHintSetPriority(hintTrisolve, 1);
    ocrHintSetAccess0Slot(hintTrisolve, 0);
    ocrHintSetAccess0Weight(hintTrisolve, 66);
    ocrHintSetAccess1Slot(hintTrisolve, 1);
    ocrHintSetAccess1Weight(hintTrisolve, 34UL);
    ocrEdtTemplateCreateWithHints(&templateTrisolve, trisolve_task, 4, 2, hintTrisolve);

    ocrGuid_t hintUpdateDiag;
    ocrHintCreate(&hintUpdateDiag);
    ocrHintSetPriority(hintUpdateDiag, 2);
    ocrHintSetAccess0Slot(hintUpdateDiag, 0);
    ocrHintSetAccess0Weight(hintUpdateDiag, 66);
    ocrHintSetAccess1Slot(hintUpdateDiag, 1);
    ocrHintSetAccess1Weight(hintUpdateDiag, 34UL);
    ocrEdtTemplateCreateWithHints(&templateUpdate, update_diagonal_task, 5, 2, hintUpdateDiag);

    ocrGuid_t hintUpdateNonDiag;
    ocrHintCreate(&hintUpdateNonDiag);
    ocrHintSetPriority(hintUpdateNonDiag, 3);
    ocrHintSetAccess0Slot(hintUpdateNonDiag, 0);
    ocrHintSetAccess0Weight(hintUpdateNonDiag, 66);
    ocrHintSetAccess1Slot(hintUpdateNonDiag, 1);
    ocrHintSetAccess1Weight(hintUpdateNonDiag, 34UL);
    ocrEdtTemplateCreateWithHints(&templateUpdateNonDiag, update_nondiagonal_task, 5, 3, hintUpdateNonDiag);
