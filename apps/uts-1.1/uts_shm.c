...
#if 1
/* RAG -- needed to avoid int overflow for the larger problems */
  counter_t nNodes, maxTreeDepth;               /* tree stats  */
  counter_t nLeaves;
#else
...
/* RAG_DEBUG */
  if (debug & 2) {
    printf("PHC %1d->%1d %2.2d %2.2d\n",
           parent->type,childType,parentHeight, numChildren);
  }
#endif
...
void showStats(double elapsedSecs) {
  int i;
#if 1
/* RAG -- uts_shm.c has different size and leaf counts than upc and mpi */
/* RAG -- because int overflows on larger problems */
