...
int addNode_c(int (*edge_adder)(int, int), void (*depth_setter)(int, int), int fanout, int fanin)
{
  ...
  int mydepth = g_maxID + 1;
  for(i = 0; i < fanin; ++i)
    {

      nref src = findMatch_c();
      if(src !=-1){
	int add_depth = edge_adder(src, newid); //depth at which newid is added
	if(add_depth< mydepth) mydepth = add_depth;
	edgecounter++;
      }
      else
	{
  ...
  while(1)
    {
      ...
      int retval =  addNode_c(edge_adder, depth_setter, fanout, fanin);
      if(retval != -1)
  	return;
      if(maxFanIn > 0)
  	maxFanIn = maxFanIn - 1;
