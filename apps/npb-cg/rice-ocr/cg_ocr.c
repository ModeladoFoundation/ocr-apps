...
#include "cg_ocr.h"
// args class->na, class->blk, a, x, edt_continue, 4
void conj_grad(u64 n, u64 blk, ocrGuid_t a, ocrGuid_t x, ocrGuid_t cont, u32 dep)
{
    ...
    ocrDbCreate(&r,&ptr,sizeof(double)*n,DB_PROP_NO_ACQUIRE,currentAffinity,NO_ALLOC);
    ocrDbCreate(&p,&ptr,sizeof(double)*n,DB_PROP_NO_ACQUIRE,currentAffinity,NO_ALLOC);
